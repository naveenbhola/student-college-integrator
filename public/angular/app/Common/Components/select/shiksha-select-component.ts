...
let optionsTemplate = `
    <ul *ngIf="optionsOpened && options && options.length > 0 && !firstItemHasChildren"
        class="ui-select-choices ui-select-choices-content ui-select-dropdown dropdown-menu" role="menu">
...
@Component({
  ...
  <div tabindex="0"
    ...
    <div class="ui-select-match"
      ...
      <span tabindex="-1"
          ...
          style="outline: 0; width: 100%; white-space: nowrap;">
        <span *ngIf="active.length <= 0 && !inputValue " class="ui-select-placeholder text-muted cmsFont">{{placeholder}}</span>
        <span *ngIf="active.length <= 0 && inputValue" class="ui-select-placeholder text-muted cmsFont">{{inputValue}}</span>
        <span *ngIf="active.length > 0"  class="ui-select-match-text pull-left"
              [ngClass]="{'ui-select-allow-clear': allowClear && active.length > 0}"
              [innerHTML]="active[0].text"
              style="text-overflow: clip; width: inherit; overflow: hidden;"
              ></span>
    ...
    <input type="text" autocomplete="false" tabindex="-1"
           ...
           id="shiksha-select-admin-single"
           *ngIf="inputMode"
  ...
  <div tabindex="0"
    ...
    <input type="text"
           ...
           spellcheck="false"
           class="form-control ui-select-search"
           id="shiksha-select-admin-multiple"
           placeholder="{{active.length <= 0 ? placeholder : ''}}"
           role="combobox">
    ${optionsTemplate}
  </div>
...
export class ShikshaSelectComponent implements OnInit {
  @Input() public allowClear:boolean = false;
  @Input() public placeholder:string = '';
  @Input() public idField:string = 'id';
  @Input() public textField:string = 'text';
  @Input() public multiple:boolean = false;
  @Input() public allowOpenSearch:boolean = false;
  ...
  public inputEvent(e:any, isUpMode:boolean = false):void {
    ...
    if (!isUpMode && e.keyCode === 13) {      
      if (this.active.indexOf(this.activeOption) === -1) {
        if(this.activeOption || !this.allowOpenSearch){
           this.selectActiveMatch();
           this.behavior.next();
        }else{
          this.hideOptions();
          this.focusToInput(stripTags(this.inputValue));
          this.element.nativeElement.querySelector('.ui-select-container').focus();
          this.doEvent('selected',this.inputValue); 
          this.active = [];
    ...
    if (target) {
      ...
      setTimeout(() =>{this.behavior.filter(new RegExp(escapeRegexp(this.inputValue), 'ig'));},1000);
    }
  }
...
export class Behavior {
  ...
  public actor: ShikshaSelectComponent;
  public constructor(actor:ShikshaSelectComponent) {
    this.actor = actor;
  }
...
export class GenericBehavior extends Behavior implements OptionsBehavior {
  ...
  public prev():void {
    ...
    if(index <= 0 && !this.actor.infiniteScroll){
      this.actor.activeOption = '';
    }else{
  ...
  public next():void {
    let index = this.actor.options.indexOf(this.actor.activeOption);
    if(index + 1 > this.actor.options.length - 1 && !this.actor.infiniteScroll)
    {
       this.actor.activeOption = this.actor.options[index];
    }
    else
    {
        this.actor.activeOption = this.actor
