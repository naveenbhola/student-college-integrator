...
<html>
   ...
   <body>
	  ...
	  <div style="background:#f9f9f9; border:1px solid #eee; width:400px; margin:20px auto; padding:50px 50px 25px 50px;">
		 <form method="POST" action="/shikshaSchema/ShikshaSchema/Login" id='loginForm' onsubmit="return submitForm()">
		...
		function submitForm()
		{
			...
			if (hasError) {
			   return false;
				   ...
				   $.post('/shikshaSchema/ShikshaSchema/doLogin',{'username':email,'mpassword':(password)},function(data) {
					   if(parseInt(data) > 1) {
						   window.location = '/shikshaSchema/ShikshaSchema';
					   }
					   else {
