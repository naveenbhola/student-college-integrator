...
<?php		$i=1;
		...
		$headingArr = array("#","ErrorMessage","sourceFile", "LineNo", "URL", "Error Msg", "Error Code", "Occurences", "OccurenceTime");
		echo "<tr>";
		...
		foreach($data as $key=>$value)
		{
			echo "<tr>";
			echo "<td>".($key+1)."</td>";
			echo "<td>".html_escape($value['Query'])."</td>";
			echo "<td>".html_escape($value['filename'])."</td>";
			echo "<td>".html_escape($value['line_num'])."</td>";
			echo "<td>".html_escape($value['url'])."</td>";
			echo "<td>".html_escape($value['error_msg'])."</td>";
			echo "<td>".html_escape($value['error_code'])."</td>";
			echo "<td><a href='javascript:void(0);' onclick=\"showURLList('".$value['module_name']."','".$value['controller_name']."', '".$value['method_name']."', '".$filters['fromdate']."', '".$filters['todate']."', '".$filters['module']."', '".$value['filename']."', '".$value['line_num']."'); return false;\">".html_escape($value['Occurences'])."</a></td>";
