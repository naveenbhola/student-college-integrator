...
class SlowPages extends AppMonitorAbstract
{
	...
	function trends($moduleName = "")
	{
		...
		$trends = $this->model->getSlowPageTrends($selectedModule, $fromDate,$toDate);
		$uniqueTrends = $this->model->getUniqueSlowPageTrends($selectedModule, $fromDate,$toDate);
	...
	function getRealTimeData($moduleName, $realtimeDate, $isAjax = 0, $rowNum = -1, $yesterday = FALSE)
	{
		...
		$realtimeData = $this->model->getRealTimeSlowPageData($moduleName, $realtimeDate,$excludeCron);	
		...
		$result = $this->model->getTotalRealtimeSlowPages($moduleName, $realtimeDate, $realtimeDataParams,$excludeCron);
