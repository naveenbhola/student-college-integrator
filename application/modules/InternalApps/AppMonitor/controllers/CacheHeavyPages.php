...
class CacheHeavyPages extends AppMonitorAbstract
{
	...
	function trends($moduleName = "")
	{
		...
		$displayData = array();
		$displayData['dashboardType'] = ENT_DASHBOARD_TYPE_CACHE;
		$displayData['reportType'] = 'trends';
		$displayData['selectedModule'] = $selectedModule;
		$this->getModulesLinks($displayData, '/AppMonitor/CacheHeavyPages', 'trends');
		...
		$trends = $this->model->getCacheTrends($selectedModule, $fromDate,$toDate);
		$uniqueTrends = $this->model->getUniqueCacheTrends($selectedModule, $fromDate,$toDate);
	...
	function pagetrends($moduleName = "")
	{
		...
		$displayData['selectedModule'] = $selectedModule;
		$this->getModulesLinks($displayData, '/AppMonitor/CacheHeavyPages', 'pagetrends');
	...
	function realtime($moduleName = "", $reportType = 'realtime', $realtimeDate = '')
	{
		...
		$displayData['selectedModule'] = $selectedModule;
		$this->getModulesLinks($displayData, '/AppMonitor/CacheHeavyPages', $reportType);
		...
		$displayData['tableHeading'] = 'Cache Heavy Pages';
		$displayData['ajaxURL'] = '/AppMonitor/CacheHeavyPages/getRealTimeData';
	...
	function getRealTimeData($moduleName, $realtimeDate, $isAjax = 0, $rowNum = -1, $yesterday = FALSE)
	{
		...
		$realtimeData = $this->model->getRealTimeCacheData($moduleName, $realtimeDate, $excludeCron);		
		...
		$result = $this->model->getTotalRealtimeCachePages($moduleName, $realtimeDate, $realtimeDataParams, $excludeCron);
		$data['resultTable'] = $this->load->view('AppMonitor/slow/resultTable', array('realtimedata' => $result, 'dtype' => ENT_DASHBOARD_TYPE_CACHE), TRUE);
	...
	function detailedreport($moduleName = '')
	{
		...
		$displayData['selectedModule'] = $selectedModule;
		$this->getModulesLinks($displayData, '/AppMonitor/CacheHeavyPages', 'detailedreport');
		
		$displayData['sorters'] = array('size' => 'Average Size','hits' => 'Number of Hits', 'threshold' => '> Threshold');
		$displayData['ajaxURL'] = "/AppMonitor/CacheHeavyPages/getDetailedReportData";
	...
	function getDetailedReportData()
	{
		...
		$data = $this->model->getCachePagesDetailedData($filters);
		...
		foreach($data as $row) {
			...
			$groupedData[$key]['total_cache'] += ($row['total_hits'] * $row['average_cache'])/100;
			$groupedData[$key]['above_threshold'] += $row['num_above_threshold'];
			
			$sumHits += $row['total_hits'];
			$sumCache += ($row['total_hits'] * $row['average_cache'])/100;
		...
		$finalData[$allKey]['total_cache'] = $sumCache;
		$finalData[$allKey]['above_threshold'] = $sumAboveThreshold;
		
		$finalData += $groupedData;
		
		foreach($finalData as $key => $val) {
			$finalData[$key]['avg'] = ($finalData[$key]['total_cache'] / $finalData[$key]['hits']) * 100;
		}
	
		$displayData = array();
		$displayData['data'] = $finalData;
		$displayData['dashboard'] = ENT_DASHBOARD_TYPE_CACHE;
		$displayData['dtype'] = ENT_DASHBOARD_TYPE_CACHE;
		$displayData['filters'] = $filters;
		$displayData['selectedModule'] = $module;
		$displayData['ajaxURLList'] = "/AppMonitor/CacheHeavyPages/getURLList";
	...
	function diffReport($moduleName = '')
	{
		...
		$displayData['selectedModule'] = $selectedModule;
		$this->getModulesLinks($displayData, '/AppMonitor/CacheHeavyPages', 'diffReport');
		
		$displayData['ajaxURL'] = "/AppMonitor/CacheHeavyPages/getDiffReportData";
	...
	function getDiffReportData(){
		...
		if($selectedModule == 'shiksha'){
			$dateRange1Data = $this->model->getDataForMonitoringDiff($from1,$to1,$selectedModule,'CacheHeavyPages');
		}else {
			$dateRange1Data = $this->model->getDataForMonitoringDiff($from1,$to1,$modules[$selectedModule],'CacheHeavyPages');
		}
 		
		if($selectedModule == 'shiksha'){
			$dateRange2Data = $this->model->getDataForMonitoringDiff($from2,$to2,$selectedModule,'CacheHeavyPages');
		}else {
			$dateRange2Data = $this->model->getDataForMonitoringDiff($from2,$to2,$modules[$selectedModule],'CacheHeavyPages');
		}
