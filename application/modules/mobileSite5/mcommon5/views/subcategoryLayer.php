...
<header id="page-header" class="clearfix">
	...
	<a id="subcategoryOverlayClose" href="javascript:void(0);" onclick="setTimeout(function() {clearAutoSuggestorForSubcat();}, 1000);" data-rel="back"><i class="head-icon-b"><span class="icon-arrow-left" aria-hidden="true"></span></i></a>
        <h3>Choose Course</h3>
    </div>
</header>
...
<section class="layer-wrap fixed-wrap" style="height: 100%">
    ...
    <div class="search-option2" id="autoSuggestRefine">
            <div id="searchbox2">
                ...
                <input class="searchInstitute" id="subcat-list-textbox" type="text" placeholder="Enter course name" onkeyup="subcatAutoSuggest(this.value,'state');" autocomplete="off" style="width:80% !important; display: inline-block !important;">
                <i class="icon-cl" onClick="clearAutoSuggestorForSubcat();">&times;</i>
            </div>
    </div>
        ...
        <section>
           ...
           <div>
               ...
               <?php foreach( $allSubCategoryArray as $subCategory ){ ?>
                       <li id="SCsLI<?php echo $subCategory['subCatId'];?>" onClick="window.location = '<?=$subCategory['URL']?>'">
                            <label id ="SCL_sLI<?php echo $subCategory['subCatId'];?>" style="cursor: pointer;">
				<?php if($subCategory['subCatName']=='Event Management' || $subCategory['subCatName']=='Aircraft Maintenance Engineering' || $subCategory['subCatName']=='Game Design and Development'){ ?>
                                <span id="<?php echo 'SCsN'.$subCategory['subCatId'];?>" style="line-height:16px;"><?php echo $subCategory['subCatName'];?> in <?php echo $subCategory['catName'];?></span>
				<?php }else{ ?>
				<span id="<?php echo 'SCsN'.$subCategory['subCatId'];?>" style="line-height:16px;"><?php echo $subCategory['subCatName'];?></span>
				<?php } ?>
                            </label>
                       </li>
               <?php } ?>
                        <li href="javascript:void(0);" id="not-found-subcat-list" style="display:none;">
...
<script>
function subcatAutoSuggest(searchedString,type){
	var str='s';
	jQuery("#not-found-subcat-list").hide();
	...
	var a=0, b=0, c=0;
	if(searchedString!=''){
		$('span[id^="SCsN"]').each( function(ind) {
	...
	for(var i=0,x=showSuggestionsCount; i<highPriorityArray.length && i<x; i++)	{
		$('#SCsLI'+highPriorityArray[i]['id']).show();
	}

	if(i<10){
		var valuesLeft = showSuggestionsCount - i;
		for(var j=0,k=showSuggestionsCount; j<lowPriorityArray.length && j<valuesLeft; j++)	{
			$('#SCsLI'+lowPriorityArray[j]['id']).show();
		}		
	}
	if(searchedString!=''){
		if(highPriorityArray.length==0 && lowPriorityArray.length==0 && str=='s'){
			jQuery("#not-found-subcat-list").show();
		}
	}
}


function clearAutoSuggestorForSubcat(){
    jQuery('#subcat-list-textbox').val('');
    subcatAutoSuggest('','state');
}
