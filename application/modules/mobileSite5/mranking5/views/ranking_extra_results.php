...
if(!empty($partial_results)){
	...
	if(!empty($locationResults) || !empty($examResults)){
		...
		?>
			<div class="confirmation-msg" style="margin:25px 0px 10px 10px;">
	...
	if(!empty($locationResults)){
		...
		$this->load->view('ranking_subheading', $locationPageData);
		$this->load->view('ranking_table_container', $locationPageData);
	}
	...
	if(!empty($examResults)){
		...
		$this->load->view('ranking_subheading', $examPageData);
		$this->load->view('ranking_table_container', $examPageData);
	}
}
