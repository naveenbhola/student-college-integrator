...
<div id="wrapper" data-role="page"  >
	...
	<header id="page-header" class="clearfix">
		...
		<h1>
		    ...
		    if(count($results)>1) {
			    echo count($results)." courses similar to ".'<span>"'.displayTextAsPerMobileResolution($temp_course['similarname'],2,true).'"</span>';
		    } else {
			    echo count($results)." course similar to ".'"'.displayTextAsPerMobileResolution($temp_course['similarname'],2,true).'"';
		    }
		    ?>	
		    </div>
		</h1>
	...
	foreach($results as $course):
		if(isset($course['addReqInfoVars']) && $course['addReqInfoVars']!=''){
			$addReqInfoVarsFinal = $course['addReqInfoVars'];
		}
	endforeach;
			...
			if($course['ispaid']=="TRUE" || true){?>
				<p>
					<?php if(in_array($course['id'],$applied_courses)){?>
						<a class="button blue small disabled" href="javascript:void(0);" id="request_e_brochure<?=$course['id'];?>"><i class="icon-pencil" aria-hidden="true"></i><span>Request E-Brochure</span></a>
						<?php }else{ ?>
						<a class="button blue small" href="javascript:void(0);" id="request_e_brochure<?=$course['id'];?>" onClick="trackReqEbrochureClick('<?=$course['id'];?>');validateRequestEBrochureFormData('<?=$institute_id;?>','<?=$course['rebLocallityId'];?>','<?=$course['rebCityId'];?>','<?=$course['isMultiLocation'];?>','<?php echo $course['id'];?>','','<?php echo $trackingPageKeyId;?>');"><i class="icon-pencil" aria-hidden="true"></i><span>Request E-Brochure</span></a>
						<?php } ?>
				</p>
			    
			   <form action="/muser5/MobileUser/renderRequestEbrouchre" method="post" id="brochureForm<?=$course['id'];?>">
						<input type="hidden" name="courseAttr" value = "<?php echo $addReqInfoVarsFinal; ?>" />
						<input type="hidden" name="current_url" value = "<?php echo url_base64_encode($course['current_url']); ?>" /> 
						<input type="hidden" name="referral_url" value = "<?php echo url_base64_encode($course['referral_url']); ?>" />
						...
						<input type="hidden" name="tracking_keyid" id="tracking_keyid<?=$course['id'];?>" value="">
				</form>
			     <?php } ?>
		    </div>
		</dd>
...
function trackReqEbrochureClick(courseId){
		try{
		_gaq.push(['_trackEvent', 'HTML5_Search_Similar_Courses_Request_Ebrochure', 'click', courseId]);
		}catch(e){}
}
