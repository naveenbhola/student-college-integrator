...
<ul>
    ...
    <?php       
                $row_count = 1; 
		$result_type= 'normal';
		$startOffset = $this->input->post('start');
		$pageId = (isset($startOffset)) ? (($startOffset/10) + 1) : 1;
		foreach ($final_modified_array as $institute_id=>$courses):
			...
			foreach($courses as $course_id=>$course) {
				if($course_id != $courseDocument->getId()) {
					...
					if(checkEBrochureFunctionality($course)){
	...
	<article <?php if(in_array($courseDocument->getInstitute()->getId(), $sponsoredInstituteIds)) : $result_type='sponsored';?>style="background:#fffddc !important"<?php endif;?> class="req-bro-box clearfix"  >
		<?php if(in_array($courseDocument->getInstitute()->getId(), $sponsoredInstituteIds)):?><div style="text-align:right; color:#898884; margin-bottom:5px;">Sponsored Results</div><?php endif;?>
			...
			<div class="details" style="cursor: pointer;"> 
				...
				<a href="<?php echo $courseDocument->getURL(); ?>" 
				onclick="setCookie('currentCourse','<?php echo $courseDocument->getId();?>','',''); 
				trackSearchQueryMobile(event, '<?php echo $row_count;?>', '<?php echo $courseDocument->getURL(); ?>', '<?php echo $courseDocument->getId();?>', 'course-title', '<?php echo $result_type;?>', '<?php echo $page_id;?>');">
			...
			<section class="other-opt" style="margin-top: 10px; cursor: pointer;">
				<form action="/msearch5/Msearch/showSimilarCourses" method="post">
					...
					<input type="hidden" name="tracking_keyid" id="tracking_keyid" value="<?php echo $similardTrackingPageKeyId;?>">
