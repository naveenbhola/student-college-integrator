	...
	function registerUser($firstName, $lastName,$mobile, $email)
	{
		...
		if (is_array($this->userStatus)) {
			...
			$displayname = $firstName . rand(1, 100000);
				...
				$userarray['sourcename'] = 'MOBILE_LDB_REGISTRATION_FORM';
				...
				$CI->usermodel->trackUserLogin($signedInUser[0]['userid']);
                        ...
                        {
			    $data[0]['userId'] = $signedInUser[0]['userid'];
			    $data[0]['status'] = 'live';
			    $data[0]['sessionId'] = sessionId ();
			    $shortlistlistingmodel = $this->ci_obj->load->model ( 'listing/shortlistlistingmodel' );
