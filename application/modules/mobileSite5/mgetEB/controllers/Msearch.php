...
class Msearch extends ShikshaMobileWebSite_Controller {
	...
	private function getSearhDataForFilterandLocation() {
        ...
        function alsoOnShiksha($courseId){
                if(empty($courseId)) {
                                return ;
                }
                ...
                if(is_array($alsoViewedInstitutes) && count($alsoViewedInstitutes)) {
                                $displayData['institutes'] = array_slice($this->instituteRepository->findWithCourses($alsoViewedInstitutes),0,5);
                                ...
                                $displayData['pageType'] = $type;
                                $template_name = 'mobileSimilarCourses';
