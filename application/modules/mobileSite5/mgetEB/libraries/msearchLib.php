...
class msearchLib {
	...
	private $_CI = NULL;
	public static $number_of_result = 10;
	...
	public function makeSearchURL($base_url,$query_params) {
		...
		foreach($query_params as $key=>$value) {
			//If the Key is "keyword", we should handle a couple of elements
			if($key=='keyword'){
	...
	public function parseFacets($facets) {
		...
		foreach ($facets['location'] as $key=>$location_facet) {
			...
			foreach ($location_facet['cities'] as $key1=>$facet) {
				...
				//Add code for ZONE
				foreach ($facet['zones'] as $keyZone=>$facetZone){
					$this->location_facets[$key]['cities'][$key1]['zone'][$keyZone]['name'] = $facetZone['name'];
					$this->location_facets[$key]['cities'][$key1]['zone'][$keyZone]['value'] = $facetZone['value'];
					$temp_array['zone_id'] = $keyZone;
					$temp_array['locality_id'] = "";
					$this->location_facets[$key]['cities'][$key1]['zone'][$keyZone]['url'] = urldecode($this->makeSearchURL(SHIKSHA_HOME."/search/index", $temp_array));					

					//Add code for LOCALITIES
					foreach ($facetZone['localities'] as $keyLocality=>$facetLocality){
						$this->location_facets[$key]['cities'][$key1]['zone'][$keyZone]['locality'][$keyLocality]['name'] = $facetLocality['name'];
						$this->location_facets[$key]['cities'][$key1]['zone'][$keyZone]['locality'][$keyLocality]['value'] = $facetLocality['value'];
						$temp_array['locality_id'] = $keyLocality;
						$this->location_facets[$key]['cities'][$key1]['zone'][$keyZone]['locality'][$keyLocality]['url'] = urldecode($this->makeSearchURL(SHIKSHA_HOME."/search/index", $temp_array));					
					}
