				...
				<div style="height:auto;" onclick="window.location.href='<?php echo $course_object->getURL();?>'">
				 <div class="notify-details" style="cursor: pointer;" >
					...
					<!-- Display E-Brochure link if Course is Paid Start -->
						...
						<?php if($course_object->isPaid()=="TRUE"){?>
						    <p style="width:99%;">
							<?php if(in_array($course_object->getId(),$applied_courses)){?>
							<a class="button blue small disabled" href="javascript:void(0);" id="request_e_brochure<?=$course_object->getId();?>"><i class="icon-pencil" aria-hidden="true"></i><span>Request Brochure</span></a>
							<?php }else{ ?>
							<a  class="button blue small" href="javascript:void(0);" id="request_e_brochure<?=$course_object->getId();?>" onClick="trackReqEbrochureClick('<?php echo $course_object->getId();?>');validateRequestEBrochureFormData('<?=$course_object->getInstitute()->getId();?>','<?=$arr['rebLocallityId'.$course_object->getInstitute()->getId()];?>','<?=$arr['rebCityId'.$course_object->getInstitute()->getId()];?>','<?=$arr['isMultiLocation'.$course_object->getInstitute()->getId()];?>','<?php echo $course_object->getId();?>');"><i class="icon-pencil" aria-hidden="true"></i><span>Request Brochure</span></a>
							<?php } ?>
						    </p>
...
?>
		...
		/*function trackReqEbrochureClick(courseId){
				try{
				_gaq.push(['_trackEvent', 'HTML5_GET_EB_Single_Courses_Request_Ebrochure', 'click', courseId]);
				}catch(e){}
		}*/
		</script>
