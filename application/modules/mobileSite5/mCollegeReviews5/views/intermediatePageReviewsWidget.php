...
<?php }
	  ...
	  $iteration = ($PageNo*3)-2;
	  
	  foreach($reviewData['results'] as $courseId => $reviewDataResult) {
	      $loopCount = 0;
	    ?>  
	   <div class="colgRvwSlidrBx bordRdus3px">
	   <?php if($displayNumbers == 'yes'){	?>
            <div class="colge-rev-ct"><span><?=$iteration?></span>
                <i class="sprite-bg"></i>
	    </div>
	    <?php } ?>
            
            <div class="colgRvwNew-p1"> 
                <div class="rv_title" id="<?='institute_'.$courseId ?>">
			  <a href="<?=$reviewDataResult['instituteUrl'].'/reviews';?>"><?=$reviewDataResult['instituteName']?></a>
                </div> 
                <p class="rv_add" id="<?='city_'.$courseId ?>"><?=$reviewDataResult['cityName']?></p> 
                <p class="rv_course" id="<?='course_'.$courseId ?>"><a href="<?=$reviewDataResult['instituteUrl'].'/reviews?course='.$courseId?>"><?=$reviewDataResult['courseName']?></a></p> 
            </div>
	    <?php
	    $width = (300 * $reviewDataResult['totalReviews'])."px";
	    ?>
            <div class="colgRvwSlidrBx-inner" style='width:<?=$width?>;' stream='<?=$stream?>' baseCourse='<?=$baseCourseId?>' educationType='<?=$educationType?>' substream='<?=$substreamId?>'  courseId='<?=$courseId?>' totalReviews='<?=$reviewDataResult['totalReviews']?>'> <!--  Code for slider id need to be placed id='1' for demo-->
	      <!-- ist box starts-->
		<?php for($i=0 ;$i<$reviewDataResult['totalReviews']; $i++){   //need to be changed
		if($loopCount == 0){
		...
		?>   
		      <div class="colgRvwHP revewNav whtBx2" style='padding: 0' id='<?=$courseId.'_review_'.$i?>'>
		          <div class="revwListBx revwListBx2 whtBx bordRdus3px">
                    
			    <p class="clr"></p>
			      <div class="rv_midd"><span class="rv_midd1" id="<?=$username_id?>" style = "word-wrap: break-word;width:100%;"> <?=$username?> </span><span class="rv_midd2" id='<?=$year_id?>'>Class of <?=$yearGrad?></span>
			        <p class="clr"></p>
				  <ul class="rv_nav">
				    <li>
...
?>
			...
			<?php
		      ...
		      ?>
		      <?php if($showHelpfulText){ ?> 
				 <p id="rv_helpful_text_<?=$reviewDataResult['nextReviewId']; ?>" style="color: #c2c0c0; float: left; font-size: 12px; line-height: 22px;">Is this review helpful? <a id="helpfulFlag" name="helpfulFlag" value="Yes" style="color: #006fa2; font-weight:bold " onclick="storeHelpfulFlag(<?=$reviewDataResult['nextReviewId']; ?>,this,<?=$userId?>); return false;" href="">YES</a></p>
				 <p id="rv_thank_text_<?=$reviewDataResult['nextReviewId']; ?>" style="color: #c2c0c0; float: left; font-size: 12px; line-height: 22px; display:none;">Thank you for your vote</p>
		      <?php } else { ?>
				 <p id="rv_reviewed_text_<?=$reviewDataResult['nextReviewId']; ?>" style="color: #c2c0c0; float: left; font-size: 12px; line-height: 22px;">Thanks, You have already voted.</p>
		      <?php }
			    }
		      ?>
