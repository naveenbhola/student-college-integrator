...
class SimilarInstitutes_Model extends MY_Model
{
	...
	function getSimilarInstitutes($seed_data,$result_count,$exclusion_list)
	{
		...
		if(count($shiksha_course_ids))
		{
			...
			$this->_recommendation_log->log('DATA # '.$course_id,$this->_db->last_query());
				
			return $rows;	
		}
		else
		{
			$this->_recommendation_log->log('DATA # '.$course_id,'No matching shiksha courses');
		}
