...
<?php $i=0; foreach($courseIdArr as $key => $courseId){
  if(!empty($hasForm[$key])){
      $type = 'Apply_Online_Internal_Form';
      if($online_form_institute_seo_url[$courseId]['seo_url']!='') {
        $seoURL = str_replace('<courseName>', strtolower(seo_url($instituteList[$key]['courseName'],'-',30)), $online_form_institute_seo_url[$courseId]['seo_url']);
        $seoURL = str_replace('<courseId>', $courseId, $seoURL);
        $seo_url = SHIKSHA_HOME.$seoURL;
      }else {
        $seo_url = "/Online/OnlineForms/showOnlineForms/".$courseId;   
      }
?>
  <div class="set-col">
...
<?php } else{?>
  <div class="set-col">
    <a href="javascript:void(0);" class="apply-btn-noui" style="cursor: default;">--</a>
  </div>
  <?php } $i++;}?>
</div> 
<?php }?>