...
class TaggingCreationLib
{
	...
	function _init(){
		...
		$this->subVal2 = array('Counselling','Cut-off','Eligibility','GDPI','Dates','Pattern','Preparation','Process','Scores','Coaching Centres','colleges accepting','Answer Key','Admit Card','Selection Process','Provisional Allotment Process','Centres','Application Process','Syllabus','Notifications','Application Form','Sample Papers','Application Fee');
	...
	public function showVarients($tagName,$tagEntity){
		...
		switch ($tagEntity) {
			...
			case $this->actualTagsArray['Sub-Stream']:
	...
	*/
	public function showSubaVal1Varients($tagName,$tagEntity = ''){
		...
		foreach ($this->subVal1 as $value) {
			$finalresult[] = trim($tagName." ".$value);
			
		}

		// One Extra Varient -- Not Covered in subval1 Array
		foreach ($this->allLocationsArray as $value) {
			$finalresult[] = trim($tagName." in ".$value);
		}

		/*if($tagEntity == 'Course'){
			$modeArray = $this->CI->taggingcmsmodel->getTagsArray('Education Type');
			foreach ($modeArray as $key => $value) {
				$finalresult[] = $value['tags']." ".$tagName;
	...
	/**
		Function to Generate Course Modes for Base Course
	*/

	public function showCourseModes($tagName)
	{
		$this->CI->load->model('Tagging/taggingcmsmodel');
		$finalresult = array();
		$modeArray = $this->CI->taggingcmsmodel->getTagsArray('Education Type');
		...
		$modeArray = $this->CI->taggingcmsmodel->getTagsArray('Delivery Method');
			foreach ($modeArray as $key => $value) {
				$finalresult[] = $value['tags']." ".$tagName;
	...
	/**
		Function to Generate Course Substream/Specialization for Base Courses
	*/

	public function showCourseSubstreamsSpec($tagName,$entityName)
	{
		$this->CI->load->model('Tagging/taggingcmsmodel');
		$finalresult = array();
		$resultArray = $this->CI->taggingcmsmodel->getTagsArray($entityName);
		foreach ($resultArray as $key => $value) {
			$finalresult[$value['id']] = $tagName." ".$value['tags'];
	...
	public function showSubaVal2Varients($tagName){
		...
		foreach ($this->subVal2 as $value) {
			if($value == "colleges accepting"){
				$finalresult[] = trim($value." ".$tagName);
			}else{
				$finalresult[] = trim($tagName." ".$value);	
			}
		}
	...
	*/
	public function showSubaVal4Varients($tagName){
		...
		$finalresult[] = "courses to become a ".$tagName;
		return $finalresult;
	}
