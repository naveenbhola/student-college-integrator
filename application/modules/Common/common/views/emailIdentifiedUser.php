<div style="width:500px">
    <div style="background:#FFFFFF;border:1px solid #d6e6f6">
	    ...
	    <input type="hidden" value="<?php echo $multkeyname; ?>" id="keyName" name="keyName"/>
            ...
            <div style="padding:10px 15px">
                <div class="OrgangeFont fontSize_16p bld" id="idContent1" style="width:100%">You will be receiving alerts for the subscribed events shortly</div>
                ...
                <div>
					...
					<div style="padding-bottom:10px" id="pEnterPassedContent">
						...
						<div class="float_L OrgangeFont" style="width:300px">
							<div><input id="password" name="password" type="password" style="width: 120px;" />&nbsp;&nbsp;<a id="forgotPasswordSubmitLink" href="javascript:void(0);" onclick="return sendForgotPasswordMail();" style="font-size:11px">forgot password</a></div>
							<div style="display:none"><div style="padding-left:0px;" class="errorMsg" id="password_error"></div></div>
						</div>
						<div class="clear_L" style="font-size:1px;line-height:1px">&nbsp;</div>
					</div>
...
<script type="text/javascript">
    ...
    function userLoginActionOnCancle()
    {
		...
		if(!((typeof(askInstitute) != 'undefined') && (askInstitute.successMessage=='showLogin'))){
			if (flag_get_free_alert) {
				...
				if(document.getElementById('keyName').value=='EMAIL'){
				displayMessage('/MultipleApply/MultipleApply/showoverlay/15',500,50);
				}else if(document.getElementById('keyName').value=='SMS'){
				displayMessage('/MultipleApply/MultipleApply/showoverlay/16',500,50);
				}else{
				displayMessage('/MultipleApply/MultipleApply/showoverlay/10',500,50);
				}
				}
			} else {
				if(eventTriggered == 'followInstitute'){
    ...
    function userLoginActionOnSubmit()
    {
        try {
            ...
            document.getElementById('password_error').parentNode.style.display = '';
