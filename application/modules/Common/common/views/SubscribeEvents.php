...
<div style="width:500px">
    <form id="requestEbrochure" name="myForm" onSubmit="validate_form(); return false;">
    <input type = "hidden" name = "resolution" id = "resolution" value = ""/>
    <input type = "hidden" name = "coordinates" id = "coordinates" value = ""/>
    <input type = "hidden" name = "referer" id = "referer" value = ""/>
    <input type = "hidden" name = "loginproductname" id = "loginproductname" value = ""/>
    <input type = "hidden" name = "privacySettings" id = "privacySettings" value = ""/>
    <div style="background:#FFFFFF;border:1px solid #d6e6f6">
        ...
        <div class="titleBarLay">
            ...
            <div class="fontSize_16p bld">Subscribe Event</div>
            <div class="clear_B" style="font-size:1px;line-height:1px">&nbsp;</div>
        </div>
		    <?php if (false && empty($displayname)) { ?>
		    <div id = "fconnect_follow" style="display:block">
		    <div id="fb-root"></div>		    
		    <div class=lineSpace_10 mb8></div>
                    <div style="padding:0 15px;">
                    <!-- <fb:login-button scope="email,user_checkins,publish_stream,offline_access" on-login=onLoginFromFacebook('followInstituteDirect')>Fill in with Facebook</fb:login-button> -->
                    <a class="login-fb" href="javascript:void(0);" onclick="fb_login('onLoginFromFacebook(\'followInstituteDirect\');');"></a>
                    </div>
		    </div>
        ...
        <div style="padding:10px 15px;">
	   	...
	   	<div class="lineSpace_10">&nbsp;</div>
            	<div>Please provide following details</div>            
            	<div class="lineSpace_10">&nbsp;</div>
		<div id="categorySelectPanel" style="display:none">
			<div style="padding-bottom:10px">
           	...
           	<div style="padding-bottom:10px">
					...
					<div class="txt_align_r">First Name:<span class="redcolor">*</span> &nbsp;</div>
			</div>
                	<div style="margin-left:100px">
					<div><input value="<?php echo htmlentities($firstname);?>" id="usr_first_name"  tip="multipleapply_name" caption="First Name" validate="validateDisplayName" required="true" maxlength="50" minlength="1" profanity="true" type="text" name="usr_first_name" style="width:185px" /></div>
					<div style="display:none"><div class="errorMsg" id="usr_first_name_error" style="padding-left:3px;"></div></div>
					<div class="clear_L" style="font-size:1px;line-height:1px">&nbsp;</div>
				</div>
			</div>
			<div style="padding-bottom:10px">
				<div class="float_L" style="width:100px;line-height:20px">
					<div class="txt_align_r">Last Name:<span class="redcolor">*</span> &nbsp;</div>
				</div>
				<div style="margin-left:100px">
					<div><input value="<?php echo htmlentities($lastname);?>" id="usr_last_name"  tip="multipleapply_name" caption="Last Name" validate="validateDisplayName" required="true" maxlength="50" minlength="1" profanity="true" type="text" name="usr_last_name" style="width:185px" /></div>
					<div style="display:none"><div class="errorMsg" id="usr_last_name_error" style="padding-left:3px;"></div></div>
				<div class="clear_L" style="font-size:1px;line-height:1px">&nbsp;</div>
                	</div>
            	</div>    
            	<div style="padding-bottom:10px">
		...
		<div style="padding-bottom:10px">
			<div class="float_L" style="width:100px;line-height:20px">
        		...
        		<div style="margin-left:100px" class="OrgangeFont">
                		<div><input value="<?php echo $mobile;?>" <?php if($mobile){?> disabled="true" <?php } ?> profanity="true" id="mobile_phone" type="text" name="mobile_phone" validate="validateMobileInteger" required="true" maxlength="10" minlength="10" tip="multipleapply_cell" caption="mobile phone" style="width:185px" /></div>
		...
		<div style="padding-bottom:10px">
                ...
                <?php if (empty($displayname)) { ?>
					...
					<div id="imp_dates_captcha" style="display:block">			
						<div>
			...
			<div class="lineSpace_5">&nbsp;</div>
            <div align="center"><input type="submit" class="submitAbuseGlobal"  value="<?php if(!empty($cookiestr)) { echo 'Subscribe'; } else { echo 'Register';} ?>" id="submitRegisterFormButton" /></div>
		 <div class="lineSpace_10">&nbsp;</div>
        </div>
    ...
    <script type="text/javascript">
		...
		var checkboxAgree = document.getElementById('cAgreed');
                ...
                var checkboxMobile = document.getElementById('Mobile');
		var privacySettings = 0;
		...
		if(checkboxEmail.checked != true && checkboxMobile.checked !=true){
                        document.getElementById('check_Email_error').innerHTML = 'Please check atleast one in Privacy Settings.';
                        document.getElementById('check_Email_error').parentNode.style.display = 'inline';
                        orangeButtonDisableEnableWithEffect('submitRegisterFormButton',false);
                        return false;
                    }else{
                        document.getElementById('check_Email_error').innerHTML = '';
                        document.getElementById('check_Email_error').parentNode.style.display = 'none';
                    }
		...
		privacySettings=2;
		}if(checkboxEmail.checked && checkboxMobile.checked){
		privacySettings=3;
		}
		...
		document.getElementById('privacySettings').value=privacySettings;
                if (flagSignedUser == true ) {
		...
		var eventTitle=document.getElementById('title').value;
                ...
                sJSON += '}';
                var paraString = "reqInfofirstName="+document.getElementById('usr_first_name').value+"&reqInfolastName="+document.getElementById('usr_last_name').value+"&reqInfoPhNumber="+document.getElementById('mobile_phone').value+"&reqInfoEmail="+document.getElementById('contact_email').value+"&jSON="+sJSON+"&resolution="+document.getElementById('resolution').value+"&coordinates="+document.getElementById('coordinates').value+"&loginproductname="+document.getElementById('loginproductname').value+"&referer="+document.getElementById('referer').value+"&privacySettings="+document.getElementById('privacySettings').value+"&category="+document.getElementById('category').value+"&fromOthers="+document.getElementById('fromOthers').value+"&country="+document.getElementById('country').value+"&keyname="+document.getElementById('keyname').value+"&locationName="+escape(document.getElementById('locationName').value)+"&locationId="+document.getElementById('locationId').value;
                if (flagSignedUser == true ) {
		if(document.getElementById('homeSecurityCode')){
                    paraString +="&captchatext="+document.getElementById("homeSecurityCode").value;
			}
                }
		if(document.getElementById('keyname').value=='ALLSAEVENTS'){
                var url = '/events/Events/subscribeEvents/1/'+ eventId +'/studyabroad/'+ eventId +'/'+ eventTitle;
		}else if(document.getElementById('keyname').value=='ALLCEVENTS'){
		var url = '/events/Events/subscribeEvents/1/'+ eventId +'/category/'+ eventId +'/'+ eventTitle;
		}else if(document.getElementById('keyname').value=='HPEVENTS'){
		var url= '/events/Events/subscribeEvents/1/'+ eventId +'/homepage/'+ eventId +'/'+ eventTitle;
		}else if(document.getElementById('keyname').value=='VAEVENTS'){
                var url= '/events/Events/subscribeEvents/1/'+ eventId +'/viewall/'+ eventId +'/'+ eventTitle;
                }else if(document.getElementById('keyname').value=='course'){
                var url= '/events/Events/subscribeEvents/1/'+ eventId +'/course/'+ eventId +'/'+ eventTitle;
                }else if(document.getElementById('keyname').value=='institute'){
                var url= '/events/Events/subscribeEvents/1/'+ eventId +'/institute/'+ eventId +'/'+ eventTitle;
                }else{
		var url = '/events/Events/subscribeEvents/1/'+ eventId +'/event/'+ eventId +'/'+ eventTitle;
		}
		if(document.getElementById('keyname').value=='EVENTS') {
                                var urlTemp='/MultipleApply/MultipleApply/showoverlay/12';
                                }else{
                                var urlTemp='/MultipleApply/MultipleApply/showoverlay/10';
                                }
		var keyName=document.getElementById('keyname').value;
		
		//Code For FConnect Starts
		var action = 'followInstitute';
                eventTriggered = action;
		 var identifier_fb = document.getElementById('identifier_fb').value;        
        //paraString_facebook = 'action='+action+'&listingTypeId='+eventId+'&listingType='+keyName;
	        paraString_facebook = 'action='+action+'&listingTypeId='+eventId+'&listingType='+identifier_fb;
		//paraString_facebook = 'action='+action+'&listingTypeId='+eventId+'&listingType='+keyName;
		//alert(document.getElementById(a).getAttribute('type'));
		
		//Code For FConnect Ends
				...
				if(eventTriggered == 'followInstitute'){
                                checkFshareOverlay('loggedInState');
				}else{
				...
				displayMessage(urlTemp,500,50);
                                }	                               
                            }
                        } else if (trim(request.responseText) == 'login') {
                            closeMessage();
                            displayMessage('/MultipleApply/MultipleApply/showoverlay/9',500,260);
                            return false;
                        } else if (trim(request.responseText) == 'register') {
                            closeMessage();
                            displayMessage('/MultipleApply/MultipleApply/showoverlay/13/'+ keyName ,665,380);
                            return false;
			    ...
			    closeMessage();
			    if(eventTriggered == 'followInstitute'){
                                checkFshareOverlay('alreadySubscribed');
				}else{
	...
	function onLoad(){
		if(document.getElementById('keyname').value!='EVENTS' && document.getElementById('keyname').value!='course' && document.getElementById('keyname').value!='institute'){
...
<?php verifyCaptcha('secCodeIndex',$value,1); ?>
