...
class Prototype extends JavaScript {
	...
	function evaluate_remote_response()
  {
	...
	function form_remote_tag($options)
	{
		$options['form'] = true;
		$nameField = '';
		$displayNameField = (isset($options['name']))?true:false;
		if($displayNameField) $nameField = "name = ".$options['name'];
		return '<form action="'.$options['url'].'" '.$nameField.' onsubmit="'.$this->remote_function($options).'; return false;" method="'.(isset($options['method'])?$options['method']:'post').'"  >';
	}
