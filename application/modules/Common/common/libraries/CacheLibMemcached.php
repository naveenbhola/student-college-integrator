...
class CacheLibMemcached {
	...
	function __construct()
	{
		...
		{
			$this->_server = MEMCACHE_IP;
			$this->_server_port = MEMCACHE_PORT;
	...
	public function inc($key) {
        ...
        public function addMembersToHash($key,$members=array(),$ttl = 1800){
            $members = serialize($members);
            $this->store($key, $members,$ttl);
        }
        public function addMembersToHashForMultipleKeys($keys,$members = array(),$ttl = 1800){
            foreach($keys as &$key){
                $this->store($key,serialize($members[$key]),$ttl);
            }
            
        }
        
        public function getMembersOfHashByFieldNameWithValue($key,$fields = array()){
            $dataFromCache = $this->get($key);
            $dataFromCache = unserialize($dataFromCache);
            $returnArray = array();
            foreach($fields as $field){
                if(!empty($dataFromCache[$field])){
                    $returnArray[$field] = $dataFromCache[$field];
                }
            }
            return $returnArray;
        }
        ...
        public function getMembersOfHashByFieldNameWithValueForMultipleKeys($keys,$fields = array()){
            $dataFromCache = $this->getMulti($keys);
            $returnArray = array();
            $counter = 0;   // this is to make return data same as that of redis
            foreach ($dataFromCache as $key => &$data){
                $data = unserialize($data);
                foreach($fields as $field){
                    if(!empty($data[$field])){
                        $returnArray[$counter][$field] = $data[$field];
                    }
                }
                $counter++;
            }
