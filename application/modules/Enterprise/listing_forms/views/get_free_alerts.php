...
?>
    <div style="width:265px;<?php echo $styleForGetFreeAlert; ?>" id="before_get_free_alerts" >
        <form  name="get_free_alerts" id="get_free_alerts_frm" method="post" >
                ...
                <input type="hidden" name="loginproductname_get_free_alert" id="loginproductname_get_free_alert"  value="GET_FREE_ALERTS_INSTITUTE"/>
                <?php } elseif ($listing_type == 'course') { ?>
                <input type="hidden" name="loginproductname_get_free_alert" id="loginproductname_get_free_alert"  value="GET_FREE_ALERTS_COURSE"/>
                <?php } ?>
            ...
            <div class="bgGetFreeMid">
                <div class="bgFreeGetContent">
                    <div style="padding:0 10px 0 17px">
                        <div style="line-height:17px;padding-top:5px">Interested in studying at <span class="OrgangeFont bld"><?php echo $instituteName; ?>?</span></div>
                   ...
                   <?php if (empty($validateuser[0]['firstname'])) { ?>
                    ...
                    <div align="center"><input type="button" onclick="return run_click();" class="continueBtn" id="Im_interestBtn" value="Submit" /></div>
                    <div class="lineSpace_10">&nbsp;</div>
...
function run_click() {
    orangeButtonDisableEnableWithEffect('Im_interestBtn',true);
    LazyLoad.loadOnce([
    ...
    try{
        ...
        if (flagSignedUser) {
            ...
            if (flag == true ) {
				...
				orangeButtonDisableEnableWithEffect('Im_interestBtn',false);
                return false;
            }alert("fjgv");
        } else {
...
function validate(secCodeTextboxid,secCode,captchaId)
{
        ...
        new Ajax.Request(url, { method:'post',onSuccess:function (request){
                    if(trim(request.responseText)=='true'){
						orangeButtonDisableEnableWithEffect('Im_interestBtn',true);
                        updateRequestInfo();
                        trackEventByGA('<?php echo $googleTrackKey; ?>','LeftPanel');
						return true;
