...
<?php } ?>
		...
		<?php $this->load->view('listing_forms/listing_Toll_free_number',array('institute_id' => $details['institute_id'])); ?>
...
?>
          ...
          <div style="width:100%;">
                    <div class="contactBT" style="<?php echo $displayCourseSection; ?> padding-top:5px;padding-bottom:17px;">
                        ...
                        <div style="line-height:18px">
                            <?php
                            if(strlen($details['contact_name']) > 0){ ?>
                            <b>Name of the Person :</b>
                            <?php echo $details['contact_name']; ?><br />
                            <?php } ?>
                            ...
                            <?php
                            $contactNos = array(); if(strlen($details['contact_main_phone'])> 0) $contactNos[] =  nl2br_Shiksha(insertWbr($details['contact_main_phone'],22)); ?>
                            <?php if(strlen($details['contact_cell'])> 0) $contactNos[] =  $details['contact_cell'] ; ?>
                            <?php if(strlen($details['contact_alternate_phone'])> 0) $contactNos[] = $details['contact_alternate_phone'] ;
							...
							?>
								<div class="listingBtn" style="margin-top:10px"><a href="javascript:void(0)" onclick="gotoBtmForm(1)" style="color:#000;text-decoration:none">I want this institute to counsel me</a></div>
							<?php } } ?>
                        </div>
                    </div>
                    <div class="lineSpace_13" style="<?php echo $displayCourseSection; ?>">&nbsp;</div>
...
switch($detailPageComponents[$i]['anchor']){
    case 'gallery':
        echo '<span class="gallery"><a href="#gallery" title="'.$details['title'].' '.$detailPageComponents[$i]['title'].'">'.$shortTitle.'</a></span>'; 
        $showGallery = $i;
        break;
    case 'videos':
        echo '<span><a href="#'.$detailPageComponents[$i]['anchor'].'" title="'.$details['title'].' '.$detailPageComponents[$i]['title'].'">'.$shortTitle.'</a></span>'; 
        $showVideos = $i;
        break;
    case 'documents':
        echo '<span><a href="#'.$detailPageComponents[$i]['anchor'].'" title="'.$details['title'].' '.$detailPageComponents[$i]['title'].'">'.$shortTitle.'</a></span>'; 
...
<?php } ?>
                    ...
                    <div>
                        ...
                        $courseUrl = getSeoUrl($tmpCourList[$i]['course_id'],"course",$tmpCourList[$i]['courseTitle'],$tempOptionalArgs);
                                ...
                                ?>
                                        ...
                                        <?php } ?>
                                            <div>
                                                ...
                                                <div style="background:url(/public/images/blkdotted.gif) no-repeat left top;padding-left:18px"><a href="<?php echo (($ListingMode == 'view') ?$courseUrl:'#'); ?>" title="<?php echo $tmpCourList[$i]['courseTitle'].' Course - '.$details['institute_name']; ?>"><?php echo $tmpCourList[$i]['courseTitle']; ?></a></div>
                                            </div>
                                <?php
                                }
                                if(($totalCount > 3)) {
...
?>
	...
	<?php
		$pageKeyForAskQuestion = 'LISTING_COURSEDETAIL_ASK_INSTITUTE_POSTQUESTION';	
		$categoryId = 0;
		if(count($details['categoryArr']) == 1){
			$categoryId = $details['categoryArr'][0]['category_id'];
		}
	...
	<!-- End of ask institute -->
			<?php
            if(!isset($cmsData)){
                 $sourcearr = array('pageKeyInfo'=>'LISTING_COURSEDETAIL_BOTTOM_RELATED_QUESTION_WIDGET_');
	...
	<div class="raised_10"> 
		...
		<div class="boxcontent_10" style="background:#FFF5C4 url(/public/images/bgCounsel.png) repeat-x left top">
			<div class="wdh100">
				...
				<span style="position:relative;*top:-5px"><b>Interested in studying at <span class="OrgangeFont"><?php echo $details['title']; ?></span></b></span>&nbsp; &nbsp; &nbsp;<input type="button" class="listingBtn_1" value="I want this insitute to counsel me" onclick="gotoBtmForm(2)"></div>
