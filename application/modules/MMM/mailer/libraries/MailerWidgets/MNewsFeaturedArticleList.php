...
class MNewsFeaturedArticleList extends MailerWidgetAbstract
{
	...
	public function getData($userIds, $processingParams)
	{
		...
		if($templateId == 4757){
			...
			$articleData = $this->CI->sacontentmodel->getMultipleContentUrlAndTitleForMailer($featuredArticleIds,FALSE);
			
			$articles = array();
			foreach($featuredArticleIds as $articleId) {
			...
			$articleData = $this->CI->articlemodel->getArticlesData($featuredArticleIds,FALSE);
		...
		$widgetHTML = $this->CI->load->view('mailer/MailerWidgets/MNewsFeaturedArticleList',$data,TRUE);
		$widgetData = array();
		foreach($userIds as $userId) {
			$widgetData[$userId]['MNewsFeaturedArticleList'] = $widgetHTML;
		}
		return $widgetData;
	}
}
