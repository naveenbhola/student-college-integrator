...
google_afs_channel = '<?php echo (isset($channelId) ? $channelId : 'home_page'); ?>'; /*// enter your
  comma-separated
  channel IDs*/
  <?php 
    if(!isset($channelId) || $channelId == 'home_page') {
        $google_afs_ad = 'w5n5';
        $google_afs_adpage = ($_REQUEST['startOffSetSearch'] / 10) +1;    
    } else {
        $google_afs_ad = 'w5';
        $google_afs_adpage = 1;
    }
    $google_context[] = isset($_REQUEST['cType']) && !empty($_REQUEST['cType']) ?$_REQUEST['cType']  : '';;
    $google_context[] = isset($_REQUEST['courseLevel']) && !empty($_REQUEST['courseLevel']) ? $_REQUEST['courseLevel'] : '';
...
google_afs_qry_lnk = '<?php echo $_REQUEST['searchType']; ?>';
