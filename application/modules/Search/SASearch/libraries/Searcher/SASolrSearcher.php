...
class SASolrSearcher {
	...
	private $searchType = 'abroadlisting';
...
//                        _p(simplexml_load_string($qerOutput));
	...
	private function getQerParametersForInstitute($keyword = null,$returnArray = false){
		...
		//echo "qerResultString"._p($keyword);
		...
		{   $qerUrl = $this->config->item('qer_url');
			...
			$qerParam = array();
			$qerUrl = $qerUrl."?inkeyword=".$keyword."&output=xmlcand&action=Submit";
			$qerResultString = $this->searchServer->curl(sanitizeUrl($qerUrl));
			...
			//echo "0000000000000000======"._p($solrQueryData);
			if($returnArray == true) {
