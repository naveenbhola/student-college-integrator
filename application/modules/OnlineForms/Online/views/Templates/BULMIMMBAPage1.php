<script>
  function checkTestScore(obj){
	...
	var objects1 = new Array(key+"RollNumberAdditional",key+"ScoreAdditional",key+"DateOfExaminationAdditional",key+"RankAdditional",key+"PercentileAdditional",key+"PercentileBULMIM");
	if(obj && $(key+"1")){
...
<div class='formChildWrapper'>
	<div class='formSection'>
		<ul>		
			...
			<li>
				<h3 class="upperCase">Exam Undertaken</h3>
				</li>
				<li>
				<div class='additionalInfoLeftCol' style="width:100%">
				<label>Examination Taken: </label>
				<div class='fieldBoxLarge' style="width:500px">
				<input type='checkbox'  onClick="checkTestScore(this);"  validate="validateCheckedGroup"   required="true"   caption="tests"   name='BULMIM_testNames[]' id='BULMIM_testNames0'   value='CAT'   onmouseover="showTipOnline('Tick the appropriate box and provide test Registration and score (if available)',this);" onmouseout="hidetip();" ></input><span  onmouseover="showTipOnline('Tick the appropriate box and provide test Registration and score (if available)',this);" onmouseout="hidetip();" >CAT</span>&nbsp;&nbsp;
				<input type='checkbox'  onClick="checkTestScore(this);"  validate="validateCheckedGroup"   required="true"   caption="tests"   name='BULMIM_testNames[]' id='BULMIM_testNames1'   value='MAT'     onmouseover="showTipOnline('Tick the appropriate box and provide test Registration and score (if available)',this);" onmouseout="hidetip();" ></input><span  onmouseover="showTipOnline('Tick the appropriate box and provide test Registration and score (if available)',this);" onmouseout="hidetip();" >MAT</span>&nbsp;&nbsp;
				<input type='checkbox'  onClick="checkTestScore(this);"  validate="validateCheckedGroup"   required="true"   caption="tests"   name='BULMIM_testNames[]' id='BULMIM_testNames3'   value='XAT'     onmouseover="showTipOnline('Tick the appropriate box and provide test Registration and score (if available)',this);" onmouseout="hidetip();" ></input><span  onmouseover="showTipOnline('Tick the appropriate box and provide test Registration and score (if available)',this);" onmouseout="hidetip();" >XAT</span>&nbsp;&nbsp;
				<input type='checkbox'   onClick="checkTestScore(this);" validate="validateCheckedGroup"   required="true"   caption="tests"   name='BULMIM_testNames[]' id='BULMIM_testNames2'   value='CMAT'     onmouseover="showTipOnline('Tick the appropriate box and provide test Registration and score (if available)',this);" onmouseout="hidetip();" ></input><span  onmouseover="showTipOnline('Tick the appropriate box and provide test Registration and score (if available)',this);" onmouseout="hidetip();" >CMAT</span>&nbsp;&nbsp;
				<input type='checkbox'   onClick="checkTestScore(this);" validate="validateCheckedGroup"   required="true"   caption="tests"   name='BULMIM_testNames[]' id='BULMIM_testNames4'   value='ATMA'     onmouseover="showTipOnline('Tick the appropriate box and provide test Registration and score (if available)',this);" onmouseout="hidetip();" ></input><span  onmouseover="showTipOnline('Tick the appropriate box and provide test Registration and score (if available)',this);" onmouseout="hidetip();" >ATMA</span>&nbsp;&nbsp;
				<?php if(isset($BULMIM_testNames) && $BULMIM_testNames!=""){ ?>
				<script>
				    objCheckBoxes = document.forms["OnlineForm"].elements["BULMIM_testNames[]"];
				    var countCheckBoxes = objCheckBoxes.length;
				    for(var i = 0; i < countCheckBoxes; i++){
					      objCheckBoxes[i].checked = false;

					      <?php $arr = explode(",",$BULMIM_testNames);
				...
				<div style='display:none'><div class='errorMsg' id= 'BULMIM_testNames_error'></div></div>
				</div>
				</div>
			</li>
			...
			<li id="cmat2" style="display:none">
				...
				<div class='fieldBoxLarge'>
				<input type='text' name='cmatPercentileBULMIM' id='cmatPercentileBULMIM'  validate="validateFloat"    caption="Percentile"   minlength="1"   maxlength="5"     tip="Mention your percentile in the exam. If you don't know your percentile, enter <b>NA.</b>"   value=''   allowNA='true' />
				<?php if(isset($cmatPercentileBULMIM) && $cmatPercentileBULMIM!=""){ ?>
				  <script>
				      document.getElementById("cmatPercentileBULMIM").value = "<?php echo str_replace("\n", '\n', $cmatPercentileBULMIM );  ?>";
				      document.getElementById("cmatPercentileBULMIM").style.color = "";
				  </script>
				<?php } ?>
				
				<div style='display:none'><div class='errorMsg' id= 'cmatPercentileBULMIM_error'></div></div>
				</div>
				</div>
			</li>
...
I hereby certify that the information provided in the Registration Form is complete, accurate and true to the
best of my knowledge and belief. I have carefully read the contents of the prospectus together with
enclosures/inserts and agree to all the terms and conditions mentioned therein. If admitted, I agree to abide
by the rules and regulations of BULMIM as may be enforced from time to time. In case any of the
information found to be incorrect, false or misrepresented, I may be expelled or/and my candidature may be
cancelled from the institute ay my cost and expenses.

				</div>
				<div class='additionalInfoLeftCol'>
				<label>I agree to the terms stated above: </label>
				<div class='fieldBoxLarge'>
				<input type='checkbox'  validate="validateChecked" checked   required="true"   caption="Please check to accept terms"   name='BULMIM_agreeToTerms[]' id='BULMIM_agreeToTerms0'   value='1'    title="I agree to the terms stated above"   onmouseover="showTipOnline('Please check to accept terms',this);" onmouseout="hidetip();" ></input><span  onmouseover="showTipOnline('Please check to accept terms',this);" onmouseout="hidetip();" ></span>&nbsp;&nbsp;
				<?php if(isset($BULMIM_agreeToTerms) && $BULMIM_agreeToTerms!=""){ ?>
				<script>
				    objCheckBoxes = document.forms["OnlineForm"].elements["BULMIM_agreeToTerms[]"];
				    var countCheckBoxes = objCheckBoxes.length;
				    for(var i = 0; i < countCheckBoxes; i++){
					      objCheckBoxes[i].checked = false;

					      <?php $arr = explode(",",$BULMIM_agreeToTerms);
						    for($x=0;$x<count($arr);$x++){ ?>
							  if(objCheckBoxes[i].value == "<?php echo $arr[$x];?>") {
								  objCheckBoxes[i].checked = true;
							  }
			      ...
			      <?php } ?>
				<div style='display:none'><div class='errorMsg' id= 'BULMIM_agreeToTerms0_error'></div></div>
				</div>
				</div>
			</li>
  ...
  function showMoreGroups(groupId,maxAllowed){
    ...
    for(var j=0; j<5; j++){
		checkTestScore(document.getElementById('BULMIM_testNames'+j));
	}
