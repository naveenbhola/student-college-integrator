<script>
  function checkTestScore(obj){
	...
	var objects1 = new Array(key+"RollNumberAdditional",key+"ScoreAdditional",key+"DateOfExaminationAdditional",key+"RankAdditional",key+"PercentileAdditional",key+"PercentileIBA");
	if(obj && $(key+"1") && $(key+"2")){
	...
	<div class='formSection'>
		<ul>
			...
			<li>
				...
				<input type='checkbox'  onClick="checkTestScore(this);"  validate="validateCheckedGroup"    caption="tests"   name='IBA_testNames[]' id='IBA_testNames0'   value='CAT'   onmouseover="showTipOnline('Tick the appropriate box and provide test Registration and score (if available)',this);" onmouseout="hidetip();" ></input><span  onmouseover="showTipOnline('Tick the appropriate box and provide test Registration and score (if available)',this);" onmouseout="hidetip();" >CAT</span>&nbsp;&nbsp;
				<input type='checkbox'  onClick="checkTestScore(this);"  validate="validateCheckedGroup"  caption="tests"   name='IBA_testNames[]' id='IBA_testNames1'   value='XAT'     onmouseover="showTipOnline('Tick the appropriate box and provide test Registration and score (if available)',this);" onmouseout="hidetip();" ></input><span  onmouseover="showTipOnline('Tick the appropriate box and provide test Registration and score (if available)',this);" onmouseout="hidetip();" >XAT</span>&nbsp;&nbsp;
				
				<input type='checkbox'  onClick="checkTestScore(this);"  validate="validateCheckedGroup"  caption="tests"   name='IBA_testNames[]' id='IBA_testNames2'   value='CMAT'     onmouseover="showTipOnline('Tick the appropriate box and provide test Registration and score (if available)',this);" onmouseout="hidetip();" ></input><span  onmouseover="showTipOnline('Tick the appropriate box and provide test Registration and score (if available)',this);" onmouseout="hidetip();" >CMAT</span>&nbsp;&nbsp;
				<input type='checkbox'  onClick="checkTestScore(this);"  validate="validateCheckedGroup"  caption="tests"   name='IBA_testNames[]' id='IBA_testNames3'   value='MAT'     onmouseover="showTipOnline('Tick the appropriate box and provide test Registration and score (if available)',this);" onmouseout="hidetip();" ></input><span  onmouseover="showTipOnline('Tick the appropriate box and provide test Registration and score (if available)',this);" onmouseout="hidetip();" >MAT</span>&nbsp;&nbsp;
				
				<?php if(isset($IBA_testNames) && $IBA_testNames!=""){ ?>
				<script>
				    objCheckBoxes = document.forms["OnlineForm"].elements["IBA_testNames[]"];
				    var countCheckBoxes = objCheckBoxes.length;
				    for(var i = 0; i < countCheckBoxes; i++){
					      objCheckBoxes[i].checked = false;

					      <?php $arr = explode(",",$IBA_testNames);
				...
				<div style='display:none'><div class='errorMsg' id= 'IBA_testNames_error'></div></div>
				</div>
				</div>
			</li>
			...
			<li id='cmat2' style='display:none;border-bottom:1px dotted #c0c0c0; padding-bottom:15px'>
				...
				<div class='fieldBoxLarge'>
				    <input type='text' name='cmatPercentileIBA' id='cmatPercentileIBA'         tip="Mention your Percentile in the exam. If you don't know your percentile, enter NA."   value='' allowNA='true' caption='percentile'  minlength=1  maxlength=5   validate='validateFloat'  />
				    <?php if(isset($cmatPercentileIBA) && $cmatPercentileIBA!=""){ ?>
				    <script>
					document.getElementById("cmatPercentileIBA").value = "<?php echo str_replace("\n", '\n', $cmatPercentileIBA );  ?>";
					document.getElementById("cmatPercentileIBA").style.color = "";
				    </script>
				    <?php } ?>
	    
				    <div style='display:none'><div class='errorMsg' id= 'cmatPercentileIBA_error'></div></div>
				</div>
			    </div>
		...
		<ul>
			...
			<li>
				...
				<div class='fieldBoxLarge' style="width:620px; color:#666666; font-style:italic">
					I hereby declare that I provided correct, complete and accurate information in this application form. I have carefully read the Information and Guidelines to Applicants. I understand and agree that any misinterpretation, false information or omission of facts in my application will lead to the denial of admission, cancellation of the admission or expulsion from the PGDM programme offered at IBA at any stage.
				</div>
				<div class='additionalInfoLeftCol'>
				<label>I agree to the terms stated above: </label>
				<div class='fieldBoxLarge'>
				<input type='checkbox'  validate="validateChecked" checked   required="true"   caption="Please check to accept terms"   name='IBA_agreeToTerms[]' id='IBA_agreeToTerms1'   value='1'    title="I agree to the terms stated above"   onmouseover="showTipOnline('Please check to accept terms',this);" onmouseout="hidetip();" ></input><span  onmouseover="showTipOnline('Please check to accept terms',this);" onmouseout="hidetip();" ></span>&nbsp;&nbsp;
				<?php if(isset($IBA_agreeToTerms) && $IBA_agreeToTerms!=""){ ?>
				<script>
				    objCheckBoxes = document.forms["OnlineForm"].elements["IBA_agreeToTerms[]"];
				    var countCheckBoxes = objCheckBoxes.length;
				    for(var i = 0; i < countCheckBoxes; i++){
					      objCheckBoxes[i].checked = false;
					      <?php $arr = explode(",",$IBA_agreeToTerms);
					      error_log($IBA_agreeToTerms);
						    for($x=0; $x<count($arr); $x++) { ?>
							  if(objCheckBoxes[i].value == "<?php echo $arr[$x];?>") {
								  objCheckBoxes[i].checked = true;
							  }
			      ...
			      <?php } ?>
				<div style='display:none'><div class='errorMsg' id= 'IBA_agreeToTerms1_error'></div></div>
				</div>
				</div>
			</li>
...
<script>
    ...
    for(var j=0; j<4; j++){
		checkTestScore(document.getElementById('IBA_testNames'+j));
	}
