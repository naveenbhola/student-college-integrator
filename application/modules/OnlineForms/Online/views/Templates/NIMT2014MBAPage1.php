<script>
  function checkTestScore(obj){
	...
	if(obj.value == "CAT" || obj.value == "XAT" || obj.value == "GMAT"){ 
	...
	<div class='formSection'>
		<ul>
			...
			<li style="width:100%">
				...
				<input onClick="checkTestScore(this);" type='checkbox' name='testNamesNIMT[]' id='testNamesNIMT0'   value='CAT'    onmouseover="showTipOnline('Tick the appropriate box and provide the registration number, test date and score (if available)',this);" onmouseout="hidetip();" /><span  onmouseover="showTipOnline('Tick the appropriate box and provide the registration number, test date and score (if available)',this);" onmouseout="hidetip();" >CAT</span>&nbsp;&nbsp;
				<input onClick="checkTestScore(this);" type='checkbox' name='testNamesNIMT[]' id='testNamesNIMT2'   value='XAT'     onmouseover="showTipOnline('Tick the appropriate box and provide the registration number, test date and score (if available)',this);" onmouseout="hidetip();" /><span  onmouseover="showTipOnline('Tick the appropriate box and provide the registration number, test date and score (if available)',this);" onmouseout="hidetip();" >XAT</span>&nbsp;&nbsp;
				<input onClick="checkTestScore(this);" type='checkbox' name='testNamesNIMT[]' id='testNamesNIMT3'   value='CMAT'     onmouseover="showTipOnline('Tick the appropriate box and provide the registration number, test date and score (if available)',this);" onmouseout="hidetip();" /><span  onmouseover="showTipOnline('Tick the appropriate box and provide the registration number, test date and score (if available)',this);" onmouseout="hidetip();" >CMAT</span>&nbsp;&nbsp;
				<input onClick="checkTestScore(this);" type='checkbox' name='testNamesNIMT[]' id='testNamesNIMT4'   value='GMAT'     onmouseover="showTipOnline('Tick the appropriate box and provide the registration number, test date and score (if available)',this);" onmouseout="hidetip();" /><span  onmouseover="showTipOnline('Tick the appropriate box and provide the registration number, test date and score (if available)',this);" onmouseout="hidetip();" >GMAT</span>&nbsp;&nbsp;
				<?php if(isset($testNamesNIMT) && $testNamesNIMT!=""){ ?>
				<script>
				    objCheckBoxes = document.forms["OnlineForm"].elements["testNamesNIMT[]"];
				    var countCheckBoxes = objCheckBoxes.length;
				    for(var i = 0; i < countCheckBoxes; i++){
					      objCheckBoxes[i].checked = false;

					      <?php $arr = explode(",",$testNamesNIMT);
				...
				<div style='display:none'><div class='errorMsg' id= 'testNamesNIMT_error'></div></div>
				</div>
				</div>
			</li>
			...
			<?php
			    if(isset($testNamesNIMT) && $testNamesNIMT!="" && strpos($testNamesNIMT,'CAT')!==false){ ?>
			    <script>
				    checkTestScore(document.getElementById('testNamesNIMT0'));
			    </script>
			<?php
			    }
			?>
			...
			<?php
			    if(isset($testNamesNIMT) && $testNamesNIMT!="" && strpos($testNamesNIMT,'XAT')!==false){ ?>
			    <script>
				    checkTestScore(document.getElementById('testNamesNIMT2'));
			    </script>
			<?php
			    }
			?>
			...
			<?php
			    if(isset($testNamesNIMT) && $testNamesNIMT!=""){ 
				    $tests = explode(',',$testNamesNIMT);
				    foreach ($tests as $test){
					  if($test=='CMAT'){
			    ?>
			    <script>
				    checkTestScore(document.getElementById('testNamesNIMT3'));
			    </script>
			<?php }
			      }
			    }
			...
			<?php
			    if(isset($testNamesNIMT) && $testNamesNIMT!="" && strpos($testNamesNIMT,'GMAT')!==false){ ?>
			    <script>
				    checkTestScore(document.getElementById('testNamesNIMT4'));
			    </script>
			<?php
			    }
			?>
			...
			<?php if($action != 'updateScore'):?>
                        <li>
                                ...
                                <div class='fieldBoxLarge'>
                                <input type='checkbox'  validate="validateChecked" checked   required="true"   caption="Please check to accept terms"   name='NIMT_agreeToTerms[]' id='NIMT_agreeToTerms0'   value='1'    title="I agree to the terms stated above"   onmouseover="showTipOnline('Please check to accept terms',this);" onmouseout="hidetip();" ></input><span  onmouseover="showTipOnline('Please check to accept terms',this);" onmouseout="hidetip();" ></span>&nbsp;&nbsp;
                                <?php if(isset($NIMT_agreeToTerms) && $NIMT_agreeToTerms!=""){ ?>
                                <script>
                                    objCheckBoxes = document.forms["OnlineForm"].elements["NIMT_agreeToTerms[]"];
                                    var countCheckBoxes = objCheckBoxes.length;
                                    for(var i = 0; i < countCheckBoxes; i++){
                                              objCheckBoxes[i].checked = false;

                                              <?php $arr = explode(",",$NIMT_agreeToTerms);
                                                    for($x=0;$x<count($arr);$x++){ ?>
                                                          if(objCheckBoxes[i].value == "<?php echo $arr[$x];?>") {
                                                                  objCheckBoxes[i].checked = true;
                                                          }
                              ...
                              <?php } ?>
                                <div style='display:none'><div class='errorMsg' id= 'NIMT_agreeToTerms0_error'></div></div>
                                </div>
                                </div>
                        </li>
                        <?php endif; ?>
