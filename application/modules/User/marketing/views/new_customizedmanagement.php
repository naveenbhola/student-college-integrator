...
if(!sendReqInfo(this)) {$('subm').disabled = false; return false; } new Ajax.Request('<?php echo $formPostUrl; ?>',{onSuccess:function(request){javascript:newuserresponse(request.responseText);}, evalScripts:true, parameters:Form.serialize(this)}); return false;" novalidate="novalidate" id = "frm1" name = "frm1">
<?php
...
?>
	<input type = "hidden" name = "pageRedirectURL" id = "pageRedirectURL" value = "<?php echo $config_data_array['REDIRECTION_URL']; ?>" />
    ...
    <input type = "hidden" name = "popupurl" id = "popupurl" value = "<?php echo $config_data_array['POPUP_URL']; ?>"/>
	<input type = "hidden" name = "pageReferer" id = "pageReferer" value = "<?php echo $_SERVER['HTTP_REFERER']; ?>" />
    <div style="width:100%">
        <div style="padding-left:20px">
            <?php if(!$hideFormHeading) { echo $config_data_array['TEXT_REGISTRATION_WIDGET']; } ?>
        </div>
        ...
        <div>
            <div id="desiredCourseDiv" style="display:none;">
                ...
                <div id="subCategory" style="margin-left:177px">
                    ...
                    <?php
                    if ($course != 'distancelearning') {
                    foreach ($newDesiredCourse_list as $value) {
                            if ($value['SpecializationId'] != '24') {
                                echo "<option value='".$value['SpecializationId']."'>".$value['CourseName']."</option>";
                            }
                        }
                    }
                    foreach ($distance_course as $value1){
                        echo "<option value='".$value1['SpecializationId']."'>".$value1['SpecializationName']."</option>";
                    }
                    ?>
            ...
            <div>
                ...
                <div style="margin-left:142px">
                    <div><input uniqueattr="MarkeingPageLayer1Submit" type="submit" class="continueBtn" value="Submit" id="subm" <?php if($logged!="No") echo 'disabled = "true"'; ?>><!--<input type="submit" id="subm" onclick="return sendReqInfo(document.getElementById('frm1'));" value="Submit" class="continueBtn" />--></div>
                </div>
                <div class="clear_L withClear">&nbsp;</div>
            </div>
            <div class="lineSpace_10">&nbsp;</div>
