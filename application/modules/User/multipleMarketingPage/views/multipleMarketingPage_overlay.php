<?php
    ...
    $date = '';
    $otherdetails = '';
    if(empty($desired)) { $desired = '-1';}
    global $testPrepCourses_requires_12th_marks;
    $testPrepFlag_secondlayer_xii_marks_details  = 'false';
    if ((in_array($desired,$testPrepCourses_requires_12th_marks)) && ($marketingpagename == 'testpreppage')) {
      $testPrepFlag_secondlayer_xii_marks_details = 'true';
    }
    ...
    //echo $local_course_flag . '=====';
    //print_r($SpecializationList);
?>
<form method="post" onsubmit="if (validateSignUpMA(document.getElementById('quicksignupForm')) === false ){ return false; } new Ajax.Request('/user/Userregistration/MultipleMarketingPage',{onSuccess:function(request){javascript:userLoginActionOnRegFormSubmit(request.responseText);}, onFailure:function(request){javascript:userLoginActionOnRegFormSubmit(request.responseText)}, evalScripts:true, parameters:Form.serialize(this)}); return false;" novalidate="novalidate" id = "quicksignupForm" name = "quicksignupForm">
...
<input type = "hidden" name = "mpagename" id = "mpagename" value = "<?php echo $local_course_flag; ?>"/>
<input type = "hidden" name = "marketingpagename" id = "marketingpagename" value = "<?php echo $marketingpagename;
?>"/>
<?php if(!empty($desired) && $desired>=25 && $desired<=34):?>
<input type = "hidden" name = "desired" id = "desired" value ="<?php echo $desired;?>"/>
<?php endif;?>
        ...
        <div style="border:1px solid #d5e4fb">
			...
			<div style="width:100%">
				<div style="padding:10px" class="fontSize_12p">
                    <div class="bld" style="padding-top:10px">Hi <label class="OrgangeFont  bld" id="display_username" ></label>, please provide additional details below to complete your profile and get maximum benefits.</div>
				</div>
			</div>
            <!--End_OverlayText-->
			...
			<div style="width:100%">
				...
				<div style="width:100%">
                            ...
                            <div class="formFieldTextRight">
								<div style="width:100%">
                                    <div id="subCategory1" style="width:200px;border:1px solid #CCC;padding:5px 0;height:55px;overflow:auto">
                                        ...
                                        <label style="display:block;padding-left:5px"><div class="float_L" style="width:22px"><input type="checkbox" name="specializationId[]" value="<?php echo $value['SpecializationId']; ?>"></div><div class="float_L" style="width:155px"><?php echo $value['SpecializationName']; ?></div><div class="clear_B">&nbsp;</div></label>
                                        <?php endforeach; ?>
                                    </div>
                        ...
                        <?php if (($local_course_flag != 'graduate_course' ) &&
($testPrepFlag_secondlayer_xii_marks_details == 'false')) { ?>			
                        <div style="width:100%">
                            <div class="formFieldTextLeft">
			...
			<div style="width:100%">
				<div align="center">
					<input type="Submit" class="btnMarketingOverlaySub" value="Submit" style="font-size:16px;font-weight:bold" uniqueattr="MultipleMarkeingPageLayer2Submit"/> &nbsp;
...
<script type="text/javascript">
    ...
    document.getElementById('display_username').innerHTML = truncateString(global_display_username,80);
    document.getElementById('display_username').setAttribute("title",global_display_username);
    document.getElementById('display_email').innerHTML = truncateString(global_display_emailid,35);
    ...
    try{
	document.getElementById('marketingpagename').value = currentPageName;
    }catch(e) {
	// alert(e);
    }
</script>
