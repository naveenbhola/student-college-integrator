...
<div id="exams_taken_block" style="<?php if(!empty($userarray['MAT']) || !empty($userarray['CAT'])): echo 'display:block'; else: echo 'display:none'; endif;?>">
    ...
    <div style="margin-left:177px">
        <div style="position:relative;*left:-4px">
			...
			<input type="checkbox" onclick="showmarksbox('ExamsTaken_cmat','ExamsTaken_cmat_span');" id="ExamsTaken_cmat" name="ExamsTaken[]" value="cmat" <?php echo isset($userarray['CMAT'])? 'checked':''?> >CMAT:
            <span style="display:none;" id="ExamsTaken_cmat_span" ><input default="Score" onClick="if(this.value =='Score'){this.value='';this.style.color = '';}" blurMethod="cmat_exm_marks();" profanity="true" type="text" minscore="0" maxscore="400" name="cmat_exm_marks" id="cmat_exm_marks" class="inputBorder" style="width:80px;color:#ada6ad;" value = "<?php echo (!empty($userarray['CMAT'])?$userarray['CMAT']:'Score');?>"/></span>
			</div>
			...
			<?php if( isset($userarray['CMAT']) ) {
                    if( !empty($userarray['CMAT'])) {
            ?>
            <script>
            document.getElementById('cmat_exm_marks').style.color = "";
            </script>
            <?php } ?>
            <script>
            document.getElementById('ExamsTaken_cmat_span').style.display = "";
            </script>
            <?php
            }
            ?>
		...
		<div>
            <div class="errorMsg" id="ExamsTaken_cmat_error" style="*padding-left:4px"></div>
        </div>
    </div>
    <div class="clear_L withClear">&nbsp;</div>
</div>
