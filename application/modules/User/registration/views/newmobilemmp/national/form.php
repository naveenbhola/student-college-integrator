...
if($registrationHelper->fieldExists('fieldOfInterest')) {
    $this->load->view('registration/fields/NewMMP/fieldOfInterest_mobile');
}
if($registrationHelper->fieldExists('desiredCourse')) {
    //$this->load->view('registration/fields/NewMMP/desiredCourse_mobile');
}
?>
<ul id="registrationFormMiddle_<?php echo $regFormId; ?>">
    <?php $this->load->view('registration/fields/NewMMP/variable/mobile'); ?>
</ul>
<ul>
...
//On Mobile Ranking and Category Pages MMP form referrer would be url of Ranking/Category Page and mmp form type 
if(isset($mmpData['display_on_page']) && in_array($mmpData['display_on_page'],array('newmmpranking','newmmpcategory')))
{
	$currentPageURL	= strip_tags(htmlentities($_SERVER['HTTP_REFERER']))."#".$mmpData['display_on_page'];
}

?>
<input type='hidden' id='referrer' name='referrer' value='<?php echo $currentPageURL; ?>' />
<input type='hidden' id='pageReferrer' name='pageReferer' value='<?php echo strip_tags(htmlentities($_SERVER['HTTP_REFERER'])); ?>' />
