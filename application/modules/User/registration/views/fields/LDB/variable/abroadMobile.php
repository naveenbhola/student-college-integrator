<div style="margin-bottom:10px;">
    <div class="counslor-evaluation-sec" id="joinShikshaLabel">
        <div class="clearfix" style="margin:5px 0 5px 5px;"><strong class="font-13">Download Brochure</strong></div>
        <p class="evaluate-title">1,42,770 brochures were downloaded on Shiksha study abroad last year. Get details about tuition fees, eligibility, living expense etc.</p></br>
        <div style="width:100%; text-align:center">
            <img src="/public/images/SASingleSignup/Download-Brochure_03.jpg" auto alt="download-brochure">
        </div>
    </div>
</div>
<div style="margin:10px 0px 15px; float: left; width:100%; color:#333; padding-bottom:5px; border-bottom:1px solid #ccc; font-size:12px;" class="wrap-title">
    Tell Us About Yourself
</div>
<div class="clearfix" style="margin-bottom:10px;"><strong class="font-13" id="sectionA">A. Study preferences</strong></div>
<?php
$univCaseFlag = false;
if($courseGroup == 'studyAbroadRevamped' && $context=='mobileRegistrationAbroad' && in_array($pageReferer, array('university','university_ranking','country_university')))
{
    $univCaseFlag = true;
    $courseCount = count($courseData);
    if($courseCount > 0 && $universityId > 0 && count($courseData) > 1)
    {
        ?>
        <div class="flLt signup-txtwidth" style="margin-bottom: 7px;width: 100%;box-sizing: border-box;<?=(count($courseList)==1)?'display:none':''?>">
            <div class="custom-dropdown" id="university_course_list_select_parent">
                <select name="university_course_list_select" class="universal-select signup-select" id="university_course_list_select" caption = "course" onchange="universityCourseSelectChangeForULP('<?= $regFormId?>');">
                    <option value="">Please select a course</option>
                    <?php foreach ($courseData as $key => $value) { ?>
                        <option <?=($courseCount==1)?'selected="selected"':''?> ispaid = "<?=($value['paid']?1:0)?>" clientCourseId = "<?=$key?>" subcategory = "<?=$value['subcategory']?>" value="<?php echo $value['desiredCourse']; ?>"><?php echo $value['name']; ?></option>
                    <?php } ?>
                </select>
            </div>
            <div>
                <div class="errorMsg error-msg" id="university_course_list_error"></div>
            </div>
        </div>
        <?php
    }
...
<section id="educationalDetailSection" <?php echo ($univCaseFlag)?'style="display:none"':'';?>>
    <div class="clearfix" style="margin-bottom:10px;"><strong class="font-13" id="sectionB">B. Education details</strong></div>
    <div id="saApplyArea">
        <?php
        if(!$univCaseFlag)
        {
            ...
            <script>
                ...
                $data = array('regFormId'=>$regFormId,
                    'ldbCourseId'=>$ldbCourse['desiredCourse'],
                    'context'=>'mobileRmcPage');
                ?>
...
<section>
    <div class="clearfix" style="margin-bottom:10px;"><strong class="font-13" id="sectionC"><?php echo ($univCaseFlag)?'B':'C';?>. Personal info</strong></div>
    <ul class="form-display customInputs">
        <?php
            ...
            <li>
                <div class="flLt" style="width:48%">
                    <div class="custom-dropdown" <?php echo $registrationHelper->getBlockCustomAttributes('isdCode'); ?>>
                        ...
                        <select class="universal-select" name="isdCode" id="isdCode_<?php echo $regFormId; ?>" onchange="shikshaUserRegistrationForm['<?php echo $regFormId; ?>'].changeStudyAbroadForm(this.value);shikshaUserRegistrationForm['<?php echo $regFormId; ?>'].changeMobileFieldMaxlength(this.value);" regFieldId="isdCode" minlength="2" maxlength="4" value="ISD Code" default="ISD Code"  <?=($disabled)?> profanity="1" <?php echo $registrationHelper->getFieldCustomAttributes('isdCode'); ?> onblur="shikshaUserRegistrationForm['<?php echo $regFormId; ?>'].validateField(this);"  style="color: #999; font-size: 12px;">
        ...
        if(isset($fields['destinationCountry'])){
            ...
            if (count($userPreferredDestinations) < 3){
                $userPreferredDestinations = array_merge($userPreferredDestinations,array($ischecked));
            }
            $userPreferredDestinations = array_unique($userPreferredDestinations);
            foreach($destinationCountry as $countryObj)
            {
                if(in_array($countryObj->getId(),$userPreferredDestinations))
                {
        ...
        <li <?php echo ($forResponse?'style="display:none;"':''); ?>>
            ...
            if(isset($fields['abroadDesiredCourse'])){
                if($forResponse) // in case of response form this is implicit
                {  ?>
