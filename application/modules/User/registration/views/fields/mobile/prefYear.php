...
?>
<div class="posrel">
	<div class="reg-form signup-fld invalid <?php if(empty($customFormFields['prefYear']['disabled'])) { echo 'sLayer'; } ?> <?php if(!empty($customFormFields['prefYear']['value'])){ echo 'filled ih'; } ?> <?php if(!empty($customFormFields['prefYear']['hidden'])){ echo 'ih'; } ?>" layerFor="prefYear" regFormId="<?php echo $regFormId; ?>" type="layer" regfieldid="prefYear" id="prefYear_block_<?php echo $regFormId; ?>" layerHeading="Preferred Year of Admission" sub-label="Select one">
		<div class="ngPlaceholder">
			Preferred Year of Admission 
			<?php  
			if(!PREF_YEAR_MANDATORY){
				?>
				<span class="optl">(Optional)</span>
				<?php
			}
			?>
		</div>
