...
In the meantime, we are sending you a free e-brochure for this course.<br><br>
<?php }?>
<?php //check if either both were attached/ any one attached (exclusively...i.e. no link at all)
if(($attachCourseBrochure && $attachUniversityBrochure)||
   ($attachCourseBrochure && $univBrochureLink == '')  ||
   ($attachUniversityBrochure && $brochureLink =='')
  )
    { ?>
        Your requested brochure<?=($attachCourseBrochure && $attachUniversityBrochure?'s are':' is')?> <u>attached</u> in this email:<br/>
        <?php if($attachCourseBrochure) { ?>
        - Course brochure for <?php echo htmlentities($courseName);?>, <?php echo htmlentities($universityName);?>, <?php echo htmlentities($countryName);?><br/>
        <?php } if($attachUniversityBrochure) { ?>
        - University brochure for <?php echo htmlentities($universityName);?>, <?php echo htmlentities($countryName);?><br/>
        <?php } ?>
<?php   } 
    else { // when links are involved
        ?>
        Your requested brochure for <?php echo htmlentities($courseName);?>, <?php echo htmlentities($universityName);?>, <?php echo htmlentities($countryName);?>:<br/>
        <br/>
        <?php
            // univ brochure
            if($attachUniversityBrochure){
                echo 'University Brochure - attached with this email<br>';
            }
            else if($univBrochureDownloadLink != ''){
                echo 'University Brochure - <a href = "'.$univBrochureDownloadLink.'<!-- #Tracking --><!-- Tracking# -->">click here to download</a><br>';
            }
            // course brochure
            if($attachCourseBrochure){
                echo 'Course Brochure - attached with this email<br>';
            }
            else if($brochureDownloadLink != ''){
                echo 'Course Brochure - <a href = "'.$brochureDownloadLink.'<!-- #Tracking --><!-- Tracking# -->">click here to download</a><br>';
            }
        ?>
<?php } ?>
<br/>
Find out more details of this course, <a href = "<?php echo $URL;?><!-- #Tracking --><!-- Tracking# -->">visit this link</a>
...
<br/>
<br/>
Looking for similar courses? Visit us at <a href="<?php echo SHIKSHA_STUDYABROAD_HOME; ?><!-- #Tracking --><!-- Tracking# -->">studyabroad.shiksha.com</a><br />
