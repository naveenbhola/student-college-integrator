<?php
	 ...
	 * 
	 */
	class MBAPredictor extends MX_Controller
	{
		...
		function __construct(){
	        ...
	        $this->mbaLib = $this->load->library('MBAPredictorLib');
...
*/
		...
		public function loadDetailedResultPage() {
		    ...
		    $iims_results_data = $this->mbaLib->startIIMPredictor(array(),$this->userId,IIM_RESULTS_YEAR,$pageNum,true,DESK_IIM_RESULTS_LIMIT);
			...
			$iims_results_data['GA_userLevel'] = $validateuser[0]['userid'] > 0 ? 'Logged In' : 'Non-Logged In';
	        ...
	        $iims_results_data['shortlistedCoursesOfUser']	= Modules::run('myShortlist/MyShortlist/getShortlistedCourse', $validateuser[0]['userid']);
	        ...
	        $iims_results_data['limit_results'] = DESK_IIM_RESULTS_LIMIT;
	    ...
	    function getIIMPredictorResultsForLoadMore(){
			...
			if(empty($pageNum) || $pageNum < 1 || !in_array($resultType, array('eligibility','inEligibility')))
				echo "";
			$iims_results_data = $this->mbaLib->startIIMPredictor(json_decode($userData,true),$this->userId,IIM_RESULTS_YEAR,$pageNum,true,DESK_IIM_RESULTS_LIMIT,$resultType);
			$iims_results_data['isAjaxCall'] = true;
			//error_log(print_r($iims_results_data,true));	
			$mappingData = $this->getMappingData();		
			$iims_results_data['fieldsmapping'] = $mappingData['fieldsMapping'];
			//$iims_results_data['board'] = $mappingData['board'];
			//$iims_results_data['gradstream'] = $mappingData['graduation_stream'];
