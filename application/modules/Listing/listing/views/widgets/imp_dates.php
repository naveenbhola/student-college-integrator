<?php
$titleIns = $details['title'];
$titleIns = (strlen($titleIns)<15)?$titleIns:substr($titleIns, 0, 12)."...";
?>
...
<input type="hidden" id="identifier_fb" autocomplete="off" value="<?php echo $identifier; ?>" />
                                <div class="wdh100">
                                    <div>                                	
                                        <h4><div class="nlt_head Fnt14 bld mb10"><b>Important Dates</b></div></h4>
			<div id="activeUnactive" style="display:none">
				  <div class="wdh100">
                                        </div>
			</div>
                                        <div class="mlr10">
							...
							foreach($eventList as $temp){
		                                        ...
		                                        $titleEvent=$temp['title'];
	                                                $titleEventTemp = strpos($titleEvent,'for');
        	                                        if($titleEventTemp)
                		                                  $titleEventNew = '<b>'.substr($titleEvent,0,($titleEventTemp-1)).'</b>'.substr($titleEvent,$titleEventTemp-1);
								...
								<input type="hidden" id="total_events" autocomplete="off" value="<?php echo $total_events; ?>" />
                                                            <div class="rw2">
                                                             <div><a href="<?php echo getSeoUrl($event_id,'event',$titleEvent); ?>"><?php echo $titleEventNew; ?></a><br>
                                                             <!-- <a href="<?php echo getSeoUrl($event_id,'event',$titleEvent); ?>"class="">View Details</a>  -->
                                                             </div>
							...
							<?php
                                                        ...
                                                        $onClickAll = "calloverlayEvents('$type_id','Listing detail page','$listing_type',2,0,2,'".addslashes($Title)."',1,'".addslashes($Title)."');return false;";
                                                        }else {
						...
						<?php } ?></div> -->
												<div class="mlr5 tar"><input type="button" value="&nbsp;" class="sprt_nlt_btn nlt_btn2" onClick="<?php echo $onClickAll; ?>;subscribeEvents('<?php echo $event_id; ?>','<?php echo addslashes($titleEvent); ?>');return false;" /></div>
                                    			<div class="ln">&nbsp;</div>
                                    			<div class="mlr5 mb10" style="margin-bottom:5px;">Share important dates with the community</div>
                                    			<div class="mlr5 tar"><input type="button" value="&nbsp;" class="sprt_nlt_btn nlt_btn3" onClick="location.replace('/events/Events/showAddEvent');" /></div>
                                            </div>                                            
                                        </div>         
                                        <div class="lh10">&nbsp;</div>
                                    </div>
...
function checkClass(from_others,total_events){
	...
	if(from_others==0){
		...
		document.getElementById('activeUnactive').innerHTML = activeHTML;
                document.getElementById('activeUnactive').style.display='block';
		if(total_events>3){
...
function viewEventsForStudyAbroad(from_others){
		...
		var days='';
		var start=0;
		var listingType = '<?php echo $listing_type; ?>';
		var typeId = <?php echo $listing_type_id; ?>;
        var countryId= document.getElementById('countryId').value;
        var country_name= document.getElementById('country_name').value;
	...
	var onClickAll = document.getElementById('onClickAll').value;
        var url = '/events/Events/getEventsForListings/1/'+ listingType +'/'+ typeId + '/' + start +'/'+ count;
			...
			var subsAllEventsHTML='';
				...
				for(var eventCounts =0;eventCounts<ajaxResp.length;eventCounts++ ){
					...
					if(ajaxResp[eventCounts].start_date_format!=ajaxResp[eventCounts].end_date_format){
                                        ...
                                        eventsHTML+='<div class="Fnt10"></div><div class="sdtBg"><div class="whiteColor">'+ ajaxResp[eventCounts].end_month +'</div><div>'+ ajaxResp[eventCounts].end_day +'</div></div>';
                                        }}else{
                                        eventsHTML+='<div class="Fnt10"></div><div class="sdtBg"><div class="whiteColor">'+ ajaxResp[eventCounts].start_month +'</div><div>'+ ajaxResp[eventCounts].start_day +'</div></div>';
                                        }
					...
					titleOfTheEvent=(ajaxResp[eventCounts].event_title).replace(new RegExp(' ','g'),'-');
                                        eventsHTML+='<a href="/events/Events/eventDetail/1/'+ ajaxResp[eventCounts].event_id +'/'+ titleOfTheEvent +'" class="">'+ eventTitleBold(ajaxResp[eventCounts].event_title) +'</a>';
...
function slideOnClick(){
	...
	var typeId = <?php echo $listing_type_id; ?>;
        ...
        var a  = new Ajax.Request (url,{ method:'post', parameters: (from_others), onSuccess: function (xmlHttp) {
                        ...
                        var eventsHTML = '';
                        for(var eventCounts =0;eventCounts<ajaxResp.length;eventCounts++ ){
							total_events=ajaxResp[eventCounts].count;
                                        eventsHTML+='<li><div class="rw1">';
                                        dateArray = ajaxResp[eventCounts].start_date.split("-");
                                        eneterdYear = dateArray[0];
                                        eneterdMonth = dateArray[1];
					...
					/*if(ajaxResp[eventCounts].start_date_format!=ajaxResp[eventCounts].end_date_format){
                                        ...
                                        //eventsHTML+='<div class="Fnt10"></div><div class="sdtBg"><div class="whiteColor">'+ ajaxResp[eventCounts].start_month +'</div><div>'+ ajaxResp[eventCounts].start_day +'</div></div>';
                                        //}
                                        eventsHTML+='<div class="Fnt10"></div><div class="sdtBg"><div class="whiteColor">'+ ajaxResp[eventCounts].month +'</div><div>'+ ajaxResp[eventCounts].date +'</div></div>';
                                        eventsHTML+='</div><input type="hidden" id="start" autocomplete="off" value="'+ ajaxResp[eventCounts].start +'" /><input type="hidden" id="from_others" autocomplete="off" value="'+ ajaxResp[eventCounts].fromOthers +'" /><input type="hidden" id="total_events" autocomplete="off" value="'+ ajaxResp[eventCounts].total_events +'" /><div class="rw2"><div>';
					titleOfTheEvent=(ajaxResp[eventCounts].title).replace(new RegExp(' ','g'),'-');
										eventsHTML+='<a href="/events/Events/eventDetail/1/'+ ajaxResp[eventCounts].id +'/'+ titleOfTheEvent +'">'+eventTitleBold(ajaxResp[eventCounts].title)+'</a><br>';
//                                       eventsHTML+='<a href="/events/Events/eventDetail/1/'+ ajaxResp[eventCounts].id +'/'+ titleOfTheEvent +'" class="">View Details</a>';                                        
					//eventsHTML+='</div><div class="drkGry">'+ ajaxResp[eventCounts].city_name +', '+ ajaxResp[eventCounts].country_name;
					/*if(ajaxResp[eventCounts].fromOthers>3){
						eventsHTML+=', '+ ajaxResp[eventCounts].start_date_format +' - '+ ajaxResp[eventCounts].end_date_format;
					}*/
					if(onClick!=''){
                                        onClick='calloverlayEvents(\''+ ajaxResp[eventCounts].id +'\',\''+ addslashes(ajaxResp[eventCounts].title) +'\',\'EVENTS\');return false;';
                                        }
						eventsHTML+='</div><a href="javascript:void(0);" onClick="'+ onClick +' subscribeEvents(\''+ ajaxResp[eventCounts].id +'\',\''+ addslashes(ajaxResp[eventCounts].title) +'\');return false;" class="vDLink_2">Subscribe</a></div><div class="clear_B"></div></li>';
                                        //eventsHTML+='</div><div class="clear_B"></div></li>';
					eventsHTML += '</div></div></li>';
                                        }
  //                      alert(eventsHTML);
                        document.getElementById('eventListByType').innerHTML = eventsHTML;
			var showingEventsHTML='';
...
function eventTitleBold(title){
var pos = title.indexOf('for');
if(pos)
    return '<b>'+title.substring(0,pos-1)+'</b>'+title.substring(pos-1);
else
    return title;
}
