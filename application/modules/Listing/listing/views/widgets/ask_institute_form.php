...
function focusBlurForWallQuestion(func,obj){
    ...
    if(func == 'focus'){
        ...
        if(element =='Ask your education related question here and get answers from Shiksha experts.'){
...
<!--<form id="askQuestionForm" name="askQuestionForm" method="get" action="<?php echo SHIKSHA_ASK_HOME_URL; ?>/messageBoard/MsgBoard/questionPostLandingPage" onsubmit="try{return validateQuestionForm_old(this,'<?php echo $postQuestionKey; ?>','formsubmit','askQuestionForm');}catch (e){ return false;}" style="margin:0;padding:0">-->
	...
	?>
	<form id="form_<?=$widget?>" onsubmit="processFloatingForm('<?=$widget?>'); return false;" novalidate>
    	...
    	<div class="errorPlace" style="display:none"><div class="errorMsg" id="ask_question_<?=$widget?>_error"></div></div>
            ...
            <div class="ask-user-info">
                <label>Courses<span>*</span></label>
                    ...
                    <select id="courseId_<?=$widget?>"  name="courseId" minlength = "10" maxlength = "10" caption = "course" class="universal-select" validate = "validateSelect" required = "true"   />
		  ...
		  <div class="errorPlace" style="display:none">
                    <div class="errorMsg" id="courseId_<?=$widget?>_error"></div>
                </div>
            </div>
            ...
            <div><button onClick="trackEventByGA('ASK_NOW_BUTTON','LISTING_DETAIL_PAGE_QNA_TAB_TOP'); processFloatingForm('<?=$widget?>'); return false;" name="submit_<?=$widget?>" id="submit_<?=$widget?>" class="orange-button" />Ask Now</button>&nbsp;&nbsp;&nbsp;<span id="cacelLinkContainer_ForAskInstitute" style="display:none;"><a href="javascript:void(0);" onClick="javascript:askInstitute.callOnFocusOnBlurFunctions('blur');" class="fontSize_12p">Cancel</a></span></div>
