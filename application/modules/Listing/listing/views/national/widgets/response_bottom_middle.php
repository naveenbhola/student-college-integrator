...
<li style="display: <?=($pageType == 'institute' || ($pageType == 'course' && $showBrochureListOnCoursePageFlag))?'':'none'?>">
<?php
if($pageType == 'course')
{
?>
<select class="universal-select" id="selected_course_<?=$widget?>" caption="course" validate="validateSelect" validation="true" onchange="reloadWidget('<?=$widget?>')" required="true">
<?php
...
{
?>
<!--<select style='width:48%;' class="universal-select" id="selected_course_<?=$widget?>" validation="false" onchange="reloadWidget('<?=$widget?>')"> -->
