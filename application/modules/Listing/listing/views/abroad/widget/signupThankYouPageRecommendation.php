...
<div id ="instTupleSec" class="univ-tab-details clearwidth">
    <h2 style="font-size:18px; margin-bottom:5px;">Students who downloaded above brochure were also interested in</h2>
    <div>	
		<ul class="tuple-cont" id = "alsoViewedRecoTuples">
			...
			$action = "signupThankYouPageRecommendation";
			$widget = "signupThankYouPageRecommendation";
			$widgetForDownload = 'signupThankYouPageRecommendation';
			$sourceForDownload = 'signupThankYouPageReco';
			...
			<div id ="<?=($widget)?>_tupleId_<?=$courseId?>" class="tuple-box <?=($widget)?>_tupleId_<?=$courseId?>">
				...
				<div class="tuple-title">
					<?php if(isset($milRecommendation) && in_array($courseId, $milRecommendation)){?>
						<p class="sponsered-text" style="display: block;">Sponsored</p>
					<?php } ?>
				...
				<div class="course-touple clearwidth">
					...
					<div class="reco-btn-col" style="">
					    ...
					    ?>
					    <a href="Javascript:void(0);" class="button-style" style="" onclick = "studyAbroadTrackEventByGA('ABROAD_ALSO_VIEWED_RECO_THANKYOU_PAGE', 'DownloadBrochure');
						loadBrochureDownloadForm('<?php echo base64_encode(json_encode($brochureDataObj))?>','/responseAbroad/ResponseAbroad/getBrochureDownloadForm','downloadBrochureFormContainer','downloadBrochure');
						processActivityTrack('<?=$courseId?>','<?=$requestedCourseId?>','<?=$courseObj['institute_id']?>','<?=$action.'ReqEBrochure'?>','<?=$widget?>','<?=$algo?>');
						"><strong>Download Brochure</strong></a>
				    ...
				    <i title="Click to ShortList" class="cate-sprite <?= $isCourseShotlistedByUser ? "added-shortlist" : "add-shortlist"?>" uniqueattr="SA_SIGNUP_THANKYOU/shortlistCourse"  onclick ="addRemoveFromShortlistedCourse('<?=$courseId?>','<?=($widget)?>','rmcSuccessPageRecommendation',<?php echo $shortlistTrackingPageKeyId; ?>);"></i>
			    </div>
			</div>
		    </li>
		    <?php }?>
