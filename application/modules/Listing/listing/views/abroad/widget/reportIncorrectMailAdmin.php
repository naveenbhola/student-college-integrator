<table width="600" border="0" cellspacing="0" cellpadding="0" align="center" style="border:8px solid rgb(73, 107, 151);">
   <tr>
      <td style="padding: 15px 0px 0px 15px;" valign="top" height="75" width="300">
         <?php 
         $params = array('autoLogin'=>false);
         echo Modules::run('systemMailer/SystemMailer/getAbroadMailerHeaderLogo', $params); 
         ?>
      </td>
            ...
            <tbody>
               <tr>
                  <td>
                     ...
                     <div width="600">
                        Data Listing Team,<br/> 
                        <p>A user has reported incorrect information on course id: <?= $courseID;?> </p>
			<?php
			$tab = explode("#",$source_page_url);
			if($tab[1]=='EntryRequirements'){
			$tabName= 'Entry requirements';	
			}elseif($tab[1]=='Fees&Expenses'){
			$tabName= 'Fees & expenses';		
			}
			?>
			<p><b>Name of Tab : </b><?= $tabName;?></p>
                        <p><b>Description of error : </b> <?=htmlentities($feedback_comment)?></p>
			<?php if($email !=''){?>
			<p><b>User Email:</b> <?=htmlentities($email)?></p>
			<?php } ?>
			<p><b>Timestamp : </b> <?=$submit_time;?></p>
            <p><b>URL Details : </b> <a href="<?php echo $source_page_url; ?>"><?=$source_page_url;?></a></p>
			<p>Please take action and manually verify if the listing needs a correction.</p>
                        Regards<br/> <b><span style='color:#F68936'>Shiksha</span> <span style='color:#3C54A4'>Study Abroad</span></b>
						<p>Get smart before you study abroad - Visit us at <a href="<?php echo SHIKSHA_STUDYABROAD_HOME; ?>">studyabroad.shiksha.com</a></p>
						   </div>
                     </div>
                  </td>
               </tr>
