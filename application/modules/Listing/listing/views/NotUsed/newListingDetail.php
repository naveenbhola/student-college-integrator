...
switch($listing_type){
    case 'course':
		echo '<script>currentPageName= "courseDetailPage";</script>';
        if(!isset($cmsData)){
            if($details['city_id'] == '')
                $city = 0;
            else
        ...
        break;
    case 'institute':
		echo '<script>currentPageName= "instituteDetailPage";</script>';
        if(!isset($cmsData)){
    ...
    $this->load->view("listing_forms/rightSideContainer",$arr);
?>
</div>
...
case 'institute':
case 'course':
    $this->load->view("listing_forms/".$listing_type."_Listing_Page");
    break;
case 'notification':
case 'scholarship':
