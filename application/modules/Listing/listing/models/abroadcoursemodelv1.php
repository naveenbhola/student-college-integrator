...
class AbroadCourseModelV1 extends ListingModelAbstract
{
    ...
    {
        error_log("Inside getDataForMultipleCourses");
        Contract::mustBeNonEmptyArrayOfIntegerValues($courseIds,'Course IDs');

        $data = $this->_getFilterwiseConsolidatedData($courseIds,$filters);
        //_p($data);
        $result =  $this->indexFilteredDataByListingIds($data,'courseId',array('general','ease_of_immigration'));
        return $result;
    }

    private function _getFilterwiseConsolidatedData($courseIds,$filters = '')
    {
        error_log("inside _getFilterwiseConsolidatedData courseIds = ");
        error_log(print_r($courseIds, TRUE));
        $validFilters = $this->validateFilters($filters);
        array_push($validFilters,'customFee_details');
        array_push($validFilters,'specialization_ids');
        array_push($validFilters,'application_eligibility');
//        _p($validFilters);
        $data = array();
...
//                    break;
                case 'customFee_details':
                    $data['customFee_details'] = $this->_getCustomFeeDetails($courseIds);
                    break;
                case 'specialization_ids':
                    $data['specialization_ids'] = $this->getAllSpecializationIdsMappingV1($courseIds);
                    break;
                case 'application_eligibility':
                    $data['application_eligibility'] = $this->_getAbroadCourseApplicationEligibiltyDetails($courseIds);
                    break;
...
//        _p($data);die;
     ...
     * select a.courseId as courseId, a.isWorkExperienceRequired as isWorkExperienceRequired, a.workExperienceValue as workExperienceValue, a.xiiCutoff as xiiCutoff, a.bachelorScoreUnit as bachelorScoreUnit, a.bachelorCutoff as bachelorCutoff, a.pgCutoff as pgCutoff from AbroadCourseApplicationEligibilityDetails a where a.status=?1 and a.courseId in (?2)
     */
    private function _getAbroadCourseApplicationEligibiltyDetails($courseIds)
    {
        $sql = 'select courseId, isWorkExperinceRequired as isWorkExperienceRequired, workExperniceValue as workExperienceValue, 12thCutoff as xiiCutoff, bachelorScoreUnit, bachelorCutoff, pgCutoff,
        workExperinceDescription as workExperienceDescription, 12thcomments as xiiCutoffComments, bachelorComments as bachelorCutoffComments, pgComments as pgCutoffComments,
         isThreeYearDegreeAccepted as threeYearDegreeAcceptedFlag, threeYearDegreeDescription as threeYearDegreeDescription 
        from abroadCourseApplicationEligibiltyDetails where status="live" and courseId in (?)';
        $data = array();
        $results = $this->db->query($sql, array($courseIds))->result_array();
        foreach($results as $row)
        {
            $data[$row['courseId']] = $row;
        }
        return $data;
    }
    /*
    ...
    {
        $sql =  //"SELECT  cd.*,".
            "SELECT cd.course_id as courseId, cd.institute_id as instituteId, cd.courseTitle as name, cd.course_level, cd.course_level_1 as level,".
            "cd.duration_value as durationValue, cd.duration_unit as durationUnit, cd.fees_value as tuition, cd.fees_unit as feeCurrency,".
            "cd.course_request_brochure_link as brochureURL, cd.profile_percentage_completion, cd.roomBoard,".
            "cd.isMealIncluded as mealFlag, cd.insurance as insurance, cd.transportation as transportation,".
//            "currency.id as feeCurrency, currency.currency_code, currency.currency_name, currency.country_id as currency_country_id,".
            "lm.pack_type as packtype, lm.listing_seo_url as seoURL,lm.listing_seo_title as title,".
            "lm.listing_seo_description as description,lm.listing_seo_keywords as keyword,".
            "lm.last_modify_date as lastModifiedDate, lm.expiry_date as expiryDate, lm.username as clientId,lm.viewCount as cumulativeViewCount,".
//            "i.institute_name,".
            "unimp.university_id as universityId,".
...
//            "LEFT JOIN currency ON (currency.id = cd.fees_unit) ".
        ...
        foreach($attributeResults as $result) {
            ...
            switch($result['caption']) {
                case 'Course Description':
                    $attributeKey = 'courseDescription';
                    break;
            }
        ...
        /**
         * adding code for attributes from course_attributes
         */
        ...
        $courseAttrRes = $this->db->query($sql, array($courseIds))->result_array();
        $courseAttr = array();
        foreach($courseAttrRes as $result){
            $courseAttr[$result['course_id']][$result['attribute']] = $result['value'];
        }
         ...
         * Start dates
         */
        //commenting as not used in new courseObject
...
//        }
        ...
        for($i=0;$i<count($results);$i++) {
            $result = $results[$i];
//            _p("result in link loop = ");_p($result);
            $courseId = $result['courseId'];
//            _p("course id in link loop");_p($courseId);
            foreach($externalLinks[$courseId] as $linkType => $link) {
                switch ($linkType){
                    case "courseWebsite":
                        $result['courseWebsiteURL'] = $link;
                        break;
                    case "courseDurationLink":
                        $result['durationURL'] = $link;
                        break;
                    case "applicationDeadlineLink":
                        $result['applicationDeadlineURL'] = $link;
                        break;
                    case "admissionWebsiteLink":
                        $result['admissionWebsiteURL'] = $link;
                        break;
                    case "englishProficiencyLink":
                        $result['englishProficiencyURL'] = $link;
                        break;
                    case "anyOtherEligibility":
                        $result['anyOtherEligibilityURL'] = $link;
                        break;
                    case "feesPageLink":
                        $result['feesPageURL'] = $link;
                        break;
                    case "scholarshipLinkCourseLevel":
                        $result['scholarshipURLCourseLevel'] = $link;
                        break;
                    case "scholarshipLinkDeptLevel":
                        $result['scholarshipURLDeptLevel'] = $link;
                        break;
                    case "scholarshipLinkUniversityLevel":
                        $result['scholarshipURLUniversityLevel'] = $link;
                        break;
                    case "facultyInfoLink":
                        $result['facultyInfoURL'] = $link;
                        break;
                    case "alumniInfoLink":
                        $result['alumniInfoURL'] = $link;
                        break;
                    case "faqLink":
                        $result['faqURL'] = $link;
                        break;
...
//            }
            foreach ($courseAttr[$courseId] as $attribute=>$attrVal){
                switch ($attribute){
                    case "examRequired":
                        $result['examRequiredDetails'] = $attrVal;
                        break;
                    case "courseAccreditation":
                        $result['accreditation'] = $attrVal;
                        break;
                    case "courseRanking":
                        $result['rankingDetails'] = $attrVal;
                        break;
                    case "curriculum":
                        $result['curriculum'] = $attrVal;
                        break;
                    case "nzqfCategorization":
                        $result['nzqfCategorization'] = $attrVal;
                        break;
                    case "AffiliatedTo":
                        $result['affiliation'] = $attrVal;
                        break;
                }
            }

            // Adding custom fees values
            //$result['customFees'] = $customFeildFees[$courseId];  //commenting for cache V1

            $results[$i] = $result;
        }

//		//add external links and attributes to original result set for general info according new course object abroadCourseV1
//        for($i=0;i<count($results);$i++){
//            $result = $results[i];
//            $courseId = $results
//        }
    ...
    private function _getCustomFeeDetails($courseIds){
        $customFeildSql = "SELECT course_id,caption,value FROM `abroad_course_custom_values_mapping` "
            ."WHERE `course_id` IN (?) "
            ."AND `valueType` = 'fees' AND `status` = 'live' ";

        $customFieldResult = $this->db->query($customFeildSql, array($courseIds))->result_array();

        $customFieldFees = array();
        foreach($customFieldResult as $customFieldResultCell){
            if($customFieldResultCell['value'] > 0){
                array_push($customFieldFees,array('courseId'=>$customFieldResultCell['course_id'],'customFeeName'=>$customFieldResultCell['caption'],
                    'customFeeValue'=>$customFieldResultCell['value']));
//                $customFieldFees[$customFieldResultCell['course_id']][] = array('caption'	=>$customFieldResultCell['caption'],
//                    'value'		=>$customFieldResultCell['value']
//                );
            }
        }

        return $customFieldFees;
    ...
    {
        $this->db->select('id,courseId, additionalRequirement');
    ...
    private function _getRmcCounsellorDetails($courseIds)
    {
        $this->db->select('
			...
			cd.course_id as courseId ',false);
...
//				"AND lea.listing_type_id IN (?) order by mt.listingPriority ASC ";
        ...
        $sql = "SELECT lea.listing_type_id as courseId, lea.examName as examNameOther, lea.examId, ".
            "lea.cutoff as examCutoff,lea.comments as examComments ".
            "FROM listingExamAbroad lea ".
            "WHERE lea.status = 'live' ".
            "AND lea.listing_type = 'course' ".
            "AND lea.listing_type_id IN (?)";
    ...
    {
        $sql =  "SELECT clm.listing_id as courseId,clm.company_order as companyOrder,clm.logo_id as companyId, ".
            "cl.company_name as companyName,cl.logo_url as logoUrl ".
    ...
    {
        $sql =  "SELECT ccp.course_id as courseId, ccp.average_work_experience as averageWorkExperience, ccp.average_gpa as averageGpa, ".
            "ccp.average_xii_percentage as averageXiiPercentage, ccp.average_gmat_score as averageGmatScore, ccp.average_age as averageAge, ".
            "ccp.percentage_international_students as percentageInternationalStudents ".
            "FROM course_class_profile ccp ".
            "WHERE ccp.status = 'live' ".
            "AND ccp.course_id IN (?) ";
    ...
    private function _getJobProfile($courseIds){
        $sql =  "SELECT cjp.course_id as courseId, cjp.percentage_employed as percentageEmployed, cjp.average_salary as averageSalary, ".
            "cjp.average_salary_currency_id as averageSalaryCurrencyId, cjp.popular_sectors as popularSectors, ".
            "cjp.internships as internships,if(cjp.isInternshipAvailable=1,'true','false') as isInternshipAvailable, cjp.internships_link as internshipsLink, ".
            "cjp.career_services_link as careerServicesLink ".
            "FROM course_job_profile cjp ".
            "WHERE cjp.status = 'live' ".
            "AND cjp.course_id IN (?) ";
    ...
    /*
     * function to get multiple specialization mapped to course
     */
    public function getAllSpecializationIdsMappingV1($courseIds){
        $this->db->select('course_id as courseId, group_concat(ldb_course_id) as ldbCourseId, university_id as universityId, category_id as categoryId ');
        $this->db->from('abroadCategoryPageData');
        $this->db->group_by('course_id');
        $this->db->where_in('course_id',$courseIds);
        $this->db->where('status','live');
