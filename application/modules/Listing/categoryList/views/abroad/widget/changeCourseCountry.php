	...
	<ul class="customInputs-large">
		<li class="form-bg">
			<div class="form-sec" id="chooseCourse">
				...
				<div class="field-wrap">
                    <div style="<?=$style?>" class="field-child-wrap chooseCourseClass">
                        <select class="universal-select" name="desiredCourse" id="desiredCourse" onchange="courseOnchangeActions();;">
                            ...
                            <?php foreach($desiredCourses as $course){
                                ...
                                <option <?=$selected?> value="<?php echo $course['SpecializationId'];?>"><?php echo $course['CourseName'];?></option>
                            <?php } ?>
                        </select>
                    </div>
					...
					<?php if($style == "") {
						...
						<div class="h-rule" id="moreOptionsDown" <?=$style_down?> ><span class="more-opt-box" onclick="moreOptionsOnclickActions();"><i class="common-sprite opt-arr-d"></i>more courses</span></div>
						<div class="h-rule" id="moreOptionsUp" <?=$style_up?> ><span class="more-opt-box" onclick="moreOptionsOnclickActions();"><i class="common-sprite opt-arr-u"></i>more courses</span></div>
				</div>
			</div>
			...
			<div class="form-sec" id="levelOfStudy" <?=$style?> >
				...
				<div class="field-wrap" style="width:440px;">
					<div class="field-child-wrap">
						<?php
						$checked = '';
						foreach($levelOfStudy as $course) {
							if($course != "PhD") {
								if(strtolower($course) == $currentCourseLevel) {
									$checked = 'checked';
								} else {
									$checked = '';
								} ?>
								<div class="columns" <?php if($course != "Certificate - Diploma") { ?> style="width:24%;" <?php } else { ?> style="width:46%;" <?php } ?> >
									<input <?=$checked?> type="radio" name="popCourse" onclick="levelOfStudyOnchangeActions();" id="<?=$course?>" value="<?=$course?>">
									<label for="<?=$course?>">
										<span class="common-sprite"></span>
										<?php if($course == "Certificate - Diploma") {
		...
		<li id="subCat">
			...
			<div class="field-wrap">
				<select class="universal-select" name="subCatSelect" id="subCatSelect" onchange="subCatOnchangeActions();">
					<option value="">All <?=$textSpecialization?> Specialization</option>
		...
		<li>
			...
			<div class="field-wrap" tabindex="1" onblur="hideCountryDropDown();" style="position: relative">
								...
								<ol>
									<li id="countryDropdownCont">
										...
										foreach($countryIdNameArray as $key=>$country) {
											...
											$flagClass = strtolower(str_replace(' ', '', $country['name']));
											?>
											<div class="country-flag-cont <?=$active?>">
												<div class="flag-main-box" onclick="checkForNumberOfCountriesSelected(this);">
													<p>
														<span class='flags flLt <?=$flagClass?>'></span> <strong><?=$country['name']?></strong>
													</p>
												</div>
				...
				<input type="button" value="Update" uniqueattr="SA_SESSION_ABROAD_CAT_PAGE/changeCourseCountryLayer" onclick="checkForErrorsCourseCountryLayer();" class="button-style big-button" style="width:100%; margin-top:20px" />
			</div>
		</li>
	</ul>
</div>