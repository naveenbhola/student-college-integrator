	...
	for($i=0;$i<count($questionList);$i++)
	{
		...
		if($questionList[$i]['status'] == 'closed')
			$closeQues = '<span class="normaltxt_11p_blk bld">Closed Question</span>';
                        ...
                        if($questionList[$i][usergroup]=='cms'){
                            $check = 'cms';
                        }else{
                            $check = 'user';
                        }
	...
	<div class="qmarked fontSize_12p">
		...
		<div class="lineSpace_18">
			<?php if($questionList[$i]['userId']==$userId){ ?>
							  <span onmouseover="javascript:try{showUserCommonOverlayForCard(this,'<?php echo $questionList[$i]['userId']; ?>');}catch(e){}" style="width:30px;display:inline;"><a href="<?php echo $arrayOfUsers[$questionList[$i]['userId']]['userProfile']; ?>"><b><?php echo $questionList[$i]['firstname'].' '.$questionList[$i]['lastname'];?></b></a></span>&nbsp;<span title="click to change your display name" style="cursor:pointer;" onClick="showUpdateUserNameImage('Edit your display name here','','','',0);"><img src="/public/images/fU.png" /></span>
						<?php }else if($questionList[$i]['vcardStatus']==1){ ?>
							  <span onmouseover="javascript:try{showUserCommonOverlayForCard(this,'<?php echo $questionList[$i]['userId']; ?>');}catch(e){}" style="width:30px;display:inline;"><span><a href="<?php echo $arrayOfUsers[$questionList[$i]['userId']]['userProfile']; ?>"><b><?php echo $questionList[$i]['firstname'].' '.$questionList[$i]['lastname'];?></b></a> </span></span>
						<?php }else{ ?>
							  <span onmouseover="javascript:try{showUserCommonOverlayForCard(this,'<?php echo $questionList[$i]['userId']; ?>');}catch(e){}" style="width:30px;display:inline;"><a href="<?php echo $arrayOfUsers[$questionList[$i]['userId']]['userProfile']; ?>"><b><?php echo $questionList[$i]['firstname'].' '.$questionList[$i]['lastname'];?></b></a></span>
						<?php } ?>
						 <span class='forA'><a href="/shikshaHelp/ShikshaHelp/upsInfo"><?php echo getTheRatingStar($questionList[$i]['level']);?></a></span> <b>asked&nbsp;</b>
						<?php
							  if($questionList[$i]['listingTitle']!='')
								  echo "<span > about <b>".$questionList[$i]['listingTitle']."</b></span><br/>";
						?>
			<a href="<?php echo $questionList[$i]['url'];?>" style="color:#000000;word-wrap:break-word">
                           <?php
                           /* if($questionList[$i][usergroup]=='cms'){
                                    $namFunc = 'showTitleDivForCMS';
                            }else{
                                    $namFunc = 'showTitleDiv';
                            }*/
                            ?>
			<?php $quesLength = strlen($questionList[$i]['msgTxt']);
				  if($quesLength<=300){ echo "<span class='' id='completeRelatedQuesDiv".$questionList[$i]['msgId']."'>".$questionList[$i]['msgTxt']."</span></a>";}
				  else {
					  echo "<span class='' id='previewQues".$questionList[$i]['msgId']."'>".substr($questionList[$i]['msgTxt'], 0, 297)."</span></a>";
					  ...
					  echo "<a href=".$questionList[$i]['url']." style='color:#000000;word-wrap:break-word'><span class='' id='completeRelatedQuesDiv".$questionList[$i]['msgId']."' style='display:none;'>".$questionList[$i]['msgTxt']."</span></a>";
				  }
                                  echo "<span>&nbsp;&nbsp;<img src='/public/images/btn_addTitle.gif' border='0' class='pointer' align='absmiddle' onclick=\"showTitleDiv('completeRelatedQuesDiv".$questionList[$i]['msgId']."',".$questionList[$i]['msgId'].",$userId,".$questionList[$i]['userId'].",'".$check."')\"></span>";
			?>
		</div>
