<?php $data['headerTitle'] = 'Campus Connect- Answer -Approved/Pending';
$this->load->view('campusRepHeader',$data);?>
	...
	?>
        <!-------------------------------->
          ...
          <ol class="ans-earn-section" id="answer_listing_main">

        <!-----------Approved-Page-------->
        <?php $this->load->view('dashboard/approvedAnswersTab');?>
        <!-------------------------------->
...
<script>
	...
	$j("#loadMoreCRBtn").click(function (e) { 
		...
		{  
			$j.post('/CA/CRDashboard/getAnswersComment',{'page': track_click}, function(data) {
			    if (data !='')
			    {   
				track_click++;
				$j("#loadMoreCRBtn").show(); 
				$j("#answer_listing_main").append(data);
				if (total_pages == track_click) {
				    $j("#loadMoreCRBtn").hide();
				    $j('#answer_listing_main li:last').attr('class','last');
				}
			    }
