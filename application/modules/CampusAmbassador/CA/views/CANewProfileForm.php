...
<?php
$loadForm2 = false;
if($userId>0 && $crIdExist=='yes' && $profileStatus=='draft')
{
	$loadForm2 = true;
}
?>
		...
		<a title="Shiksha.com" tabindex="6" href="https://www.shiksha.com">
			...
			<?php         
			$this->load->view('campusRepOnBoardHeader');
			?>
			<div class="connect-form clear-width">
				<div class="form-steps">
				<a href="javascript:void(0);" class="<?php echo ($loadForm2)?'':'active';?>"><span>1</span>Tell US about yourself</a>
				<a class="campus-sprite  step-border"></a>
				<a href="javascript:void(0);" class="<?php echo ($loadForm2)?'active':'';?>"><span>2</span>Review your College </a>
				</div>
				<?php
				//load form view
				/*if($loadForm2)
				{
				    $this->load->view('CAOnbordingForm2', array('userId'=>$userId));
				}
				else{*/
				    $this->load->view('CAOnbordingForm1', $formData);
				//}
				?>
			</div>
                </div>
