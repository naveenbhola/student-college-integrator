<div class="review_tuple clear_max">
    ...
    <?php 
        ...
        ?>
        <?php echo $linkOpen; ?>
          <p class="pic-wrap"style="background-image: url('<?php echo getImageUrlBySize($imageUrl,'medium');?>')">
        </p>
        <?php echo $linkClose; ?>
    </div>
    <div class="text_wraper">
        <div class="user_t clear_max"> <p class="fnt1_4_3">
        <?php 
            if($value['anonymousFlag']==1){
                echo 'Anonymous';
            }
        ...
        ?><span class="rating_block"><?php echo $value['overallRating'];?>/10</span> </p> <span class="r_date">Reviewed <?php echo date('d M y',  strtotime($value['addedAt']));?></span> </div>
        <div>
            <p class="fnt1_4_6"><?php
            if($value['anonymousFlag']!=1){
      ...
      <div class="text_desc"><p class="fnt_1_4_7"><?php echo htmlentities($value['reviewText']);?></p>
    <?php if($userEligibleForReviewDeletion[$value['id']]){?>
          <a href="javascript:void(0)" class="deleteReview" reviewId="<?php echo $value['id'];?>">Delete Review</a>
    <?php }?>
      </div>
    </div>
</div>