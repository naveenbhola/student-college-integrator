...
class CountryHomeLib {
    ...
    private $univObjs;
    private $univIdSets;
    private $examMasterList;
    private $univUrl;
    private $univCount;
    private $feeCourseCounts;
    private $viewCountData;
    
    function __construct(){
        $this->CI =& get_instance();
        $this->_setDependencies();
        $this->univObjs = $this->univIdSets = $this->examMasterList = $this->feeCourseCounts = array();
        $this->univCount = 0;
        $this->univUrl = '';
        $this->viewCountData = '';
    }
    ...
    function getUniversityWidgetData($countryId, $courseId = 0){
        ...
        if($this->univCount == 0){
            $result['totalCount'] = $this->univCount = $this->countryHomeModel->getCountryUniversityCount($countryId);
        }else{
            $result['totalCount'] = $this->univCount;
        }
        $abroadListingCommonLib = $this->CI->load->library('listing/abroadListingCommonLib');
        if($this->viewCountData === '')
        {
            $this->viewCountData = $abroadListingCommonLib->getViewCountCacheData('course',14);
        }
        $courseViewCountArray = $abroadListingCommonLib->getViewCountForListingsByDays(array_keys($resultTemp['courselist']),'course',14,$this->viewCountData);
        ...
        if($this->univUrl == ''){
            $result['viewAllUniversityPageUrl'] = $this->univUrl = $this->getViewAlluniversityCountryPageUrl($countryId,$resultTemp['totalCount']);
        }else{
            $result['viewAllUniversityPageUrl'] = $this->univUrl;
        }
        
        //$abroadUniversityRepository = $this->universityRepository;
        $universityArr = array();
        $this->CI->load->config('listing/studyAbroadListingCacheConfig');
        $universityObjectCountryHome = $this->CI->config->item('universityObjectCountryHome');
        if(!empty($ranUniversityIds)){
          if(empty($this->univObjs)){
            $this->univObjs = $this->universityRepository->findMultiple($ranUniversityIds,$universityObjectCountryHome);
          }else{
            $univIds = array_keys($this->univObjs);
            $newSet  = array_diff($ranUniversityIds, $univIds);
            if(!empty($newSet)){
                $this->univObjs = $this->univObjs + $this->universityRepository->findMultiple($newSet,$universityObjectCountryHome);
            }
          }
        }
        
        $universityData = array();
        foreach($ranUniversityIds as $univId)
        {
            $id = $this->univObjs[$univId]->getId();
            $universityData[$id]["university_id"]   	= $id;
            $universityData[$id]["university_name"] 	= $this->univObjs[$univId]->getName();
            $universityData[$id]["url"] 	        	= $this->univObjs[$univId]->getURL();
            $universityData[$id]["university_type"] 	= $this->univObjs[$univId]->getTypeOfInstitute();
            $universityData[$id]["establishment_year"] 	= $this->univObjs[$univId]->getEstablishedYear();
            // top viewed course must be picked
            $viewCountArr = array();        
            $tempCourseList = explode(',', $resultTemp['result'][$univId]['courseList']);
            foreach ($tempCourseList as $cid) {
                $viewCountArr[$univId][$cid] = $courseViewCountArray[$cid];
            }
            $courseList = $viewCountArr[$univId];
            arsort($courseList);
			$result[$courseId][] = $universityData[$id]["courseId"] = reset(array_keys($courseList));	
            // get location data
            $locations = $this->univObjs[$univId]->getLocation();
            if($locations)
            {
            ...
            $univPhotos = $this->univObjs[$univId]->getPhotos();
            if(count($univPhotos))
            {
    ...
    public function sortEligibilityExamForCountryHomeCourses(&$courseSolrData){
        if(empty($courseSolrData)){
            return array();
        }

        $examOrder = array();
        foreach($courseSolrData as $courseId => &$courseData)
        {
            $requestData = array();
            $requestData['LDBCourseId'] = $courseData['saCourseDesiredCourseId'];
            $requestData['categoryId'] = $courseData['saCourseParentCategoryId'];
            $requestData['courseLevel'] = $courseData['saCourseLevel1'];

            $examOrder = $this->abroadListingCommonLib->getExamOrderByDesiredCourseAndCategory($requestData);

            //sort the exams based on exam order as defined in the config
            usort($courseData['saCourseEligibilityExams'], function ($a,$b) use($examOrder)
            {
    ...
    private function getViewAlluniversityCountryPageUrl($countryId,$totalCount){
        $countryPageUrl = $this->abroadCategoryPageRequest->getURLForCountryPage($countryId);
     ...
     * please handle the case of course level detection from desired case if this function is needed
     */
    ...
    public function getCountryCourseOrder($countryId,$desiredCourses){
        return $this->countryHomeModel->getCountryCourseOrder($countryId,$desiredCourses);
    }

    public function prepareDataForFeesAndAffordabilityWidget($countryId, $courseId,$desiredCourseList){
        $this->abroadCategoryPageRequest->setData(array('countryId'=>array($countryId),'LDBCourseId'=>$courseId));
        $url = $this->abroadCategoryPageRequest->getURL();
        ...
        if(empty($this->feeCourseCounts[$countryId][$courseId])){
            $this->feeCourseCounts = $this->categorypagemodel->getLDBCourseCountsForCountries(array($countryId),$desiredCourseList); //for 1508,1509,1510 only
        }
        ...
        return array(
            'cheapest'=>array('url'=>$cheapest,'count'=>(integer)$this->feeCourseCounts[$countryId][$courseId]),
	 ...
	 */
	public function getEligibilityExamScoreWidgetData($countryId, $ldbCourseId, $desiredCoursesOnPage) {
        if(empty($this->examMasterList)){
            $this->examMasterList = $this->abroadCommonLib->getAbroadExamsMasterList('examId', 1);
        }
                $topExamsToBeShown = $this->abroadCategoryPageLib->getExamOrderByCountryAndLDBCourse(array("LDBCourseId" => $ldbCourseId,"countryId" => $countryId),$desiredCoursesOnPage);
                ...
                $mostPopularExamArticlesDisplayData = $this->_getMostPopularExamArticleDisplayData($mostPopularExamArticles, 2, $this->examMasterList);
	...
	private function _getExamAverageGraphData($listOfExams, $countryId, $ldbCourseId)
	{
		...
		foreach($listOfExams as $exam)
		{
			...
			if(isset($examAvgData) && $examAvgData !== false)
			{
				if($exam['exam'] == "CAE"){
