...
<div class="widget-wrap clearwidth" style="margin-top:20px;">
	<a id="comment_section_start" name="comment_section_start"></a>
	<h2><i class="article-sprite big-comment-icon"></i><?php echo $content['data']['commentCount'];?> <?php if($content['data']['commentCount'] > 1)echo 'Comments'; else echo 'Comment'?></h2>
	<div class="comment-section clearwidth">
		...
		if(validateComment('comment','<?php echo $contentId;?>'))
		{
		   if(checkUserStatus() == 0)
		   {
			openCommentOverlay('<?php echo $contentId;?>');
		   }
		   else
		   {
			addComment('comment','<?php echo $contentId;?>');}  }else return false;  
	...
	?>
	<div>
		<div style="width:100%;background-color:#eee;padding:10px;font-size:18px;text-align:center;clear:both;cursor:pointer;" onclick="showComments(this);">View Other Comments (<?=($totalComments)?>)</div>
		<div class="comment-box clearwidth" id="comment_box_main" style="display:none;">
			<ul id="comment_main_<?php echo $contentId; ?>">
				<?php $this->load->view('/blogs/saContent/tabloid/tabloidCommentsInner',$data);?>
			</ul>
		</div>
		<?php if($totalComments > 5):?>
		<div class="clearwidth loadMoreCommentsButton" style="display:none;">
