...
function addAltText($title,$description , $lazyLoad = 0){
    $dom = new DOMDocument;
...
function getDiscussionTopicIds($object){
	foreach ($object as $key => $value) {
		$discussionId = $value->getDiscussionTopicId();
		if(!empty($discussionId))
		{
			$discussionTopicIds[] = $value->getDiscussionTopicId();	
		}
		
	}
	return $discussionTopicIds;
}

function sortArrayAlphabatically($inputArray){
	uasort($inputArray, function($a, $b){
