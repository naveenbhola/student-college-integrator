...
class ESNationalSession extends MX_Controller
{
    private $dbHandle;
    private $categoryMapping;
    private $subCatMapping;
    private $LDBCourseMapping;	
	...
	function scanpv($startDate, $endDate)
	{
        ...
        $this->subCatMapping = $this->getSubCatMapping();
        $this->LDBCourseMapping = $this->getLDBCourseMapping();
        ...
        while($currentDate < $endDate) {
            ...
            while(true) {
                ...
                $params = array();
                $params['index'] = 'trafficdata_sessions_3';
                $params['type'] = 'session';
                ...
                if(count($search['hits']['hits']) > 0) {
                    foreach($search['hits']['hits'] as $result) {
                        ...
                        $doc = $result['_source'];
                        $doc['landingPageDoc'] = $this->getMappedDoc($doc['landingPageDoc']);
                        $doc['exitPage'] = $this->getMappedDoc($doc['exitPage']);
	...
	private function getMappedDoc($doc)
	{
		if(array_key_exists('categoryId', $doc)) {
            ...
            $docSubCategoryId = intval($doc['subCategoryId']);
            $docLDBCourseId = intval($doc['LDBCourseId']);
            $docCategoryId = $this->subCatMapping[$docSubCategoryId];
        }
        else if(array_key_exists('LDBCourseId', $doc)) {
            $docLDBCourseId = intval($doc['LDBCourseId']);
            $docSubCategoryId = $this->LDBCourseMapping[$docLDBCourseId][0];
            $docCategoryId = $this->LDBCourseMapping[$docLDBCourseId][1];
        }
        ...
        foreach(array($key1, $key2, $key3) as $key) {
            if(array_key_exists($key, $this->categoryMapping)) {
                $newMapping = $this->categoryMapping[$key];
                break;
            }    
        }
