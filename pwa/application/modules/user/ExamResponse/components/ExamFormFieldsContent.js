...
import PrefYear from './../../Response/components/PrefYear';
import ResidenceCityLocality from './../../Response/components/ResidenceCityLocality';
import ExamCourseGroup from './ExamCourseGroup';
import ExamGroupMappedFields from './ExamGroupMappedFields';
import PersonalAccountDetails from './../../Response/components/PersonalAccountDetails';
import TermsConditions from './../../Response/components/TermsConditions';
...
import { createResponse , storeResponseData } from './../../Response/actions/ResponseFormAction';
...
class ExamFormFieldsContent extends Component {
	...
	render() {
		...
		const { onlyOTP , formData , examGroupData } = this.props;
		...
		var clickId = this.props.clickId ? '&clickId='+this.props.clickId : "";
			...
			{ 
				...
				>
					<div className="scrolable-vport form-scroll" id={"sndscreenSroll_"+this.props.regFormId}>
						...
						{
							(formData.groupMappedExams && examGroupData) &&
							<ExamCourseGroup examGroupId={formData.examGroupId} groupMappedExams={formData.groupMappedExams} examGroupData={examGroupData} regFormId={this.props.regFormId} formValues={this.state.formValues} isLoggedInUser={this.props.isLoggedInUser} />
						}
				        {
				        	(!formData.groupMappedExams && examGroupData) && 
				        	<React.Fragment>
					        	<input type="hidden" name="clientCourse" value={formData.examGroupId} id={"clientCourse_"+this.props.regFormId} />
					        	<ExamGroupMappedFields examGroupId={formData.examGroupId} examGroupData={examGroupData} baseCourses={examGroupData.baseCourse} pristine={null} changeLocalityByLevel={this.changeLocalityByLevel.bind(this)} formValues={this.state.formValues} regFormId={this.props.regFormId} isLoggedInUser={this.props.isLoggedInUser} />
					        </React.Fragment>
				        }
						{
			        		showPrefYearField &&
						...
						<ResidenceCityLocality cityList={formData.cityList} baseCourse={this.state.baseCourseValue} isExamResponse={true} formValues={this.state.formValues} regFormId={this.props.regFormId} isLoggedInUser={this.props.isLoggedInUser} />
						<input type="hidden" id="regFormId" name="regFormId" value={this.props.regFormId} />
						<input type="hidden" name="listing_type" value="exam" id={"listing_type_"+this.props.regFormId} />
						...
						<input type="hidden" id="registrationSource" name="registrationSource" value="PWA_EXAM_RESPONSE" />
				    ...
				    { 
				        ...
				        <div className="btn-rltv clear">
				            <div className="Sgup-box">
				                <a className="newSks lgn2 loginScreen" onClick={this.loginClick} href={this.props.config.SHIKSHA_HOME+'/muser5/UserActivityAMP/getLoginAmpPage?fromwhere=pwaResponseLogin&listingType=exam&listingId='+formData.examGroupId+'&action='+this.props.actionType+clickId}>Already have an account? Login here</a>
				            </div>
				        </div>
				    }
				    </div>
	...
	{
		if(baseCourse != 'undefined' || baseCourse != undefined)
		{
			this.setState({...this.state, 
				baseCourseValue : baseCourse[0]
			});
		}
	}
    ...
    createResponseCall() {
		...
		var examName      = '';
		if(typeof this.props.examName !='undefined' && this.props.examName){
		...
		var data          = this.serializedFormData ? this.serializedFormData : 'listing_type='+listingType+'&action_type='+actionType+'&tracking_keyid='+trackingKeyId+'&isViewedResponse=no';
			data         +='&listing_id='+listingId+'&examName='+examName;
    ...
    getDataForRegistration() {
		...
		if(document.getElementById("responseForm_"+this.props.regFormId) !=undefined) {
        	if(document.getElementById('isdCode_'+this.props.regFormId).value!='91-2')
			{
				if(document.getElementsByName('residenceCityLocality')[0]!=undefined)
				{
					document.getElementsByName('residenceCityLocality')[0].value='';	
				}
			}
        }
...
export default connect(mapStateToProps,mapDispatchToProps)(ExamFormFieldsContent);
