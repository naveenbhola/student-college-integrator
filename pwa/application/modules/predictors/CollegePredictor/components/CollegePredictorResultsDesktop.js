...
import CollegePredictorDesktopContent from './CollegePredictorDesktopContent';
import CPDesktopFilters from './CPDesktopFilters';
import './../assets/ResultTuple.css';
import CPSelectedFilters from './CPSelectedFilters';
import TopCard from "./TopCard";
import CPZRP from "./CPZRP";
import ResultContentLoader from './../utils/ResultContentLoader';
import {getUrlParameter, makeFiltersSticky, resetGNB, removeParamFromUrl, isUserLoggedIn} from "../../../../utils/commonHelper";
import {getModifySearchUrl, getTrackingParams, makeInputObjectForResultPage, getUserFormDataFromUrl, getRegistrationPrams, getUserFormDataObjectFromUrl} from './../utils/collegePredictorHelper';
...
class CollegePredictorResultsDesktop extends React.Component{
	constructor(props){
		super(props);
		this.state = {
			isShowLoader : false
	...
	componentDidMount(){
		...
		if(this.props.location.search !== ''){
			if(!isUserLoggedIn()){
				...
				// check user loggedIn but not verified
				const axiosConfig = {
					headers: {'Content-Type': 'application/x-www-form-urlencoded'},
					withCredentials: true
				};
				getRequest(config().SHIKSHA_HOME+'/mcommon5/MobileSiteHamburgerV2/getProfileData?from=pwa',axiosConfig)
					.then((response) =>{
						if(response.data && response.data.userStatus !=='false' && response.data.userStatus && response.data.userStatus[0]['mobileverified'] !== '1'){
							this.validateUser();
						}
    ...
    validateUser = () => {
    	let formUrl = '/college-predictor';
    	let url = this.props.location.search;
		let finalUrl = (url) ? this.props.location.pathname+url : formUrl;
		let regData = getRegistrationPrams(config().SHIKSHA_HOME, predictorConfig.registrationCustomFields, finalUrl, 'desktop');
		showRegistrationFormWrapper(regData);
		
		//when user close the registration then we redirect to form1
		document.body.addEventListener( 'click',function ( event ){
		  if(event.srcElement.className === 'regClose') {
				window.location.href = (url) ? formUrl+url : formUrl;
		  };
		});
	};
	...
	makeFiltersSticky = () => {
		const filterSidebarElement = document.querySelector('.cp-filters');
		if(filterSidebarElement == null || typeof filterSidebarElement === 'undefined'){
			return;
		}
		let currScroll = document.documentElement.scrollTop;
		let footerSelector = document.getElementById('footer');
		const resultsContainer = document.querySelector('.cp-results');
    ...
    initialFetchData(params)
  	...
  	static renderLoader() {
    	...
    	return <ResultContentLoader deviceType='desktop' />;
  	}
	showZRP() {
	...
	{	
		...
		if(this.state.isShowLoader){
      		return CollegePredictorResultsDesktop.renderLoader();
    	}
		...
		let activeTab = (cpResults.tabType) ? cpResults.tabType : 'college';
					...
					<div className="pwa_container">
						<div className="college-shortlist-details">
							...
							<div className='college-shortlist-content clearFix'>
								<CPSelectedFilters {...filterData} pageUrl={basePageUrlForFilter} gaTrackingCategory={this.props.gaTrackingCategory} />
								<CPDesktopFilters {...filterData} pageUrl={basePageUrlForFilter} gaTrackingCategory={this.props.gaTrackingCategory} />
								<CollegePredictorDesktopContent currentPageNo={currentPageNo} totalResults={totalResults} resultPerPage={resultPerPage} tuplesList={(cpResults && cpResults.tuplesList) ? cpResults.tuplesList : ''} currentPageUrl={currentPageUrl} activeTab={activeTab} deviceType={this.props.deviceType} tabUrl={tabUrl} search={removeParamFromUrl('pageNo', propsLocation)} gaTrackingCategory={this.props.gaTrackingCategory} />
							</div>
						</div>
					</div>
				</div>
...
CollegePredictorResultsDesktop.defaultProps = {
	deviceType : 'desktop',
...
export default connect(mapStateToProps, mapDispatchToProps)(CollegePredictorResultsDesktop);

CollegePredictorResultsDesktop.propTypes = {
